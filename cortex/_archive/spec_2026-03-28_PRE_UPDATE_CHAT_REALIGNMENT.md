...
2. **`cortex-web`**: The web-based execution host (Vite/Lit), serving as the primary browser interface.
...
5. **`cortex-eudaemon`**: *(Deprecated)* Legacy Python-based daemon, functions now fully absorbed by `cortex-worker` via local execution adapters.
...
- **Resources**: Frontend hosts (Web) use React/Vite/Lit primitives to handle form validation, error handling, and immediate invalidation without page reloads.
