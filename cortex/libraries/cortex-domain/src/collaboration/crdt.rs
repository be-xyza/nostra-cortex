...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct AguiCrdtMutation {
    pub path: Option<String>,
    pub key: String,
    pub value_json: String, // Stored as a serialized JSON string to avoid coupling domain CRDT to A2UI schemas directly
}
...
pub struct ArtifactCrdtUpdateEnvelope {
    ...
    #[serde(default)]
    pub agui_mutations: Vec<AguiCrdtMutation>,
}
...
pub struct ArtifactCrdtState {
    ...
    #[serde(default)]
    pub agui_state: Vec<AguiCrdtStateEntry>,
    #[serde(default)]
...
pub fn init_state(artifact_id: &str, markdown: &str, updated_at: String) -> ArtifactCrdtState {
...
pub fn apply_update(
    ...
    for agui_mut in &envelope.agui_mutations {
        // Find existing entry with same path and key, or insert new
        if let Some(entry) = state
            .agui_state
            .iter_mut()
            .find(|e| e.path == agui_mut.path && e.key == agui_mut.key)
        {
            entry.value_json = agui_mut.value_json.clone();
        } else {
            state.agui_state.push(AguiCrdtStateEntry {
                path: agui_mut.path.clone(),
                key: agui_mut.key.clone(),
                value_json: agui_mut.value_json.clone(),
            });
        }
    }
...
pub fn state_hash(state: &ArtifactCrdtState) -> String {
    ...
    let mut agui_hash_parts = state
        ...
        .collect::<Vec<_>>();
    agui_hash_parts.sort();

    format!(
        "sha-lite:{}:{}:{}",
        state.applied_op_ids.len(),
        visible.join("|"),
        agui_hash_parts.join("|")
    )
}
...
mod tests {
    ...
    fn crdt_converges_three_client_sequence() {
        let mut state_a = init_state("artifact-1", "abc", "2026-01-01T00:00:00Z".to_string());
        ...
        apply_update(&mut state_a, &op_1, "2026-01-01T00:00:10Z".to_string())
            .expect("apply op1 to a");
        apply_update(&mut state_a, &op_2, "2026-01-01T00:00:11Z".to_string())
            .expect("apply op2 to a");
        apply_update(&mut state_a, &op_3, "2026-01-01T00:00:12Z".to_string())
            .expect("apply op3 to a");

        apply_update(&mut state_b, &op_3, "2026-01-01T00:00:10Z".to_string())
            .expect("apply op3 to b");
        apply_update(&mut state_b, &op_1, "2026-01-01T00:00:11Z".to_string())
            .expect("apply op1 to b");
        apply_update(&mut state_b, &op_2, "2026-01-01T00:00:12Z".to_string())
            .expect("apply op2 to b");

        apply_update(&mut state_c, &op_2, "2026-01-01T00:00:10Z".to_string())
            .expect("apply op2 to c");
        apply_update(&mut state_c, &op_3, "2026-01-01T00:00:11Z".to_string())
            .expect("apply op3 to c");
        apply_update(&mut state_c, &op_1, "2026-01-01T00:00:12Z".to_string())
            .expect("apply op1 to c");
    ...
    fn duplicate_op_is_idempotent() {
        let mut state = init_state("artifact-1", "hello", "2026-01-01T00:00:00Z".to_string());
        ...
        let first =
            apply_update(&mut state, &op, "2026-01-01T00:00:02Z".to_string()).expect("first apply");
        assert!(first.applied);
        assert!(!first.idempotent);

        let second = apply_update(&mut state, &op, "2026-01-01T00:00:03Z".to_string())
            .expect("second apply");
    ...
    #[test]
    fn state_hash_stable_across_permutations() {
        let mut state_a = init_state("artifact-1", "abc", "2026-01-01T00:00:00Z".to_string());
        let mut state_b = state_a.clone();
        ...
        apply_update(&mut state_a, &op_1, "2026-01-01T00:00:10Z".to_string()).unwrap();
        apply_update(&mut state_a, &op_2, "2026-01-01T00:00:11Z".to_string()).unwrap();

        apply_update(&mut state_b, &op_2, "2026-01-01T00:00:10Z".to_string()).unwrap();
        apply_update(&mut state_b, &op_1, "2026-01-01T00:00:11Z".to_string()).unwrap();

        assert_eq!(state_hash(&state_a), state_hash(&state_b));
    }
}
