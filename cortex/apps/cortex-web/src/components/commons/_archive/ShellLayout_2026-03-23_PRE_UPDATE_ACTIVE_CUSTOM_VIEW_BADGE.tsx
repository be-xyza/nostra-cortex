    ...
    return (
        <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
                <button
                    type="button"
                    className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/8 bg-slate-950/55 text-white/45 opacity-0 shadow-sm shadow-black/20 transition hover:border-white/15 hover:bg-white/[0.06] hover:text-white group-hover:opacity-100 group-focus-within:opacity-100"
            ...
            <DropdownMenu.Portal>
                ...
                >
                    ...
                    <DropdownMenu.Item
                        className="flex cursor-pointer items-center gap-2 rounded-xl px-2.5 py-2 text-[11px] text-white/72 outline-none transition hover:bg-white/[0.05] hover:text-white"
...
export function ShellLayout({ children }: ShellLayoutProps) {
    ...
    const customViews = useCustomViewsStore((state) => state.cache[activeSpaceId] ?? []);
    ...
    return (
        <div className={`shell-layout ${activeSpaceIds.length > 1 ? "shell-layout--meta" : ""}`}>
            ...
            <main className={`shell-main ${location.pathname.startsWith("/system") || location.pathname.startsWith("/heap") || location.pathname.startsWith("/spaces") || location.pathname.startsWith("/explore") || location.pathname.startsWith("/drafts") || location.pathname.startsWith("/activity") || location.pathname.startsWith("/pinned") || location.pathname.startsWith("/archive") ? "shell-main--full" : ""}`}>
