...
export const ProviderDashboard: React.FC = () => {
  ...
  const handleStartProviderFromDiscovery = () => {
    setSelectedProviderId("__new__");
    setForm(buildDiscoveryProviderForm(adapterStatus, discoverySelectedModel));
    setValidationResult(null);
    setSubmitError(null);
            ...
            <details className="rounded-2xl border border-border-subtle bg-surface-elevated px-4 py-3">
              ...
              <div className="mt-4 grid gap-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <label className="grid gap-2 text-sm">
                    ...
                    <p className="text-xs leading-5 text-ink-faint">
                      This is the registry key for the provider record. Built-in runtime defaults use `llm_adapter`;
                      new providers should use a template-specific id.
                    </p>
                  </label>
                  <label className="grid gap-2 text-sm">
                  ...
                  <label className="grid gap-2 text-sm">
                    ...
                    {modelSelectOptions.length > 0 ? (
                      <select
                        ...
                        className="rounded-lg border border-border-subtle bg-surface-elevated px-3 py-2 text-sm text-ink"
                      >
                        <option value="">{isRefreshingAdapterModels ? "Refreshing models..." : "Choose a model"}</option>
                    ...
                    <p className="text-xs leading-5 text-ink-faint">
                      {modelSelectOptions.length > 0
                        ? "Choose from the discovered model catalog or validate the provider again to refresh it."
                        : adapterStatusError
                ...
                <div className="rounded-xl border border-border-subtle bg-surface-elevated px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-ink">Usage bindings</p>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <label className="grid gap-2 text-sm">
                      <span className="text-ink-muted">Credential binding</span>
                      <select
                      ...
                      <p className="text-xs leading-5 text-ink-faint">
                        {visibleCredentialBindings.length > 0
                          ? "Choose the credential record to associate with this provider."
            ...
            <div className="panel-body grid gap-4">
              {selectedProvider ? (
                <div className="rounded-2xl border border-border-subtle bg-surface-elevated/70 px-4 py-4">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink-faint">Selected provider</p>
                    ...
                    <div className="rounded-xl border border-border-subtle bg-black/10 px-3 py-2 text-right">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Current key</p>
                    ...
                    <div className="panel-body">
                      ...
                      <div className="space-y-4">
                        ...
                        <details className="metric">
                          <summary className="cursor-pointer select-none text-sm">Advanced details</summary>
                          <div className="mt-2 grid grid-cols-1 gap-1">
                            {provider.adapterSetRef ? (
                              <div className="flex justify-between text-[11px]">
                                <span className="text-ink-muted lowercase">adapter set</span>
                                <span className="text-ink opacity-80">{provider.adapterSetRef}</span>
                              </div>
                            ) : null}
                            {provider.batchPolicy ? (
                              <div className="flex justify-between text-[11px]">
                                <span className="text-ink-muted lowercase">batch</span>
                                <span className="text-ink opacity-80">{formatProviderBatchSummary(provider.batchPolicy)}</span>
                              </div>
                            ) : null}
                            {provider.topology ? (
                              <>
                            ...
                            {provider.metadata && Object.keys(provider.metadata).length > 0 ? (
                              <div className="mt-2 border-t border-border-subtle pt-2">
                                <p className="text-[11px] uppercase tracking-[0.16em] text-ink-muted">Metadata</p>
                                <div className="mt-2 grid grid-cols-1 gap-1">
