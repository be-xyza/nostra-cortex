...
export const ProviderDashboard: React.FC = () => {
  ...
  return (
    <div className="provider-dashboard animate-in fade-in duration-500 p-6">
          ...
          <div className="panel-body grid gap-5">
            ...
            {selectedProvider ? (
              <div className="rounded-2xl border border-dashed border-border-strong px-4 py-4 text-sm text-ink-muted">
                Saved providers open details from their cards below. The composer stays focused on the core fields you change most often.
              </div>
            ) : (
      ...
      <div className="provider-dashboard__lead mb-8">
        <div className="provider-dashboard__lead-card">
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-ink-faint">Configured providers</p>
          <p className="mt-3 text-2xl font-semibold tracking-tight text-ink">{providers.length}</p>
          <p className="mt-2 max-w-xs text-sm leading-6 text-ink-muted">
            Saved providers are grouped below by runtime role so you can scan the system without opening each record.
          </p>
        </div>
        <div className="provider-dashboard__lead-card">
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-ink-faint">Ready now</p>
          <p className="mt-3 text-2xl font-semibold tracking-tight text-ink">{readyProviderCount}</p>
          <p className="mt-2 max-w-xs text-sm leading-6 text-ink-muted">
            Providers with an active record and an available credential. Neutral defaults stay out of the warning path.
          </p>
        </div>
        <div className="provider-dashboard__lead-card">
          <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-ink-faint">Current focus</p>
          <p className="mt-3 text-base font-semibold tracking-tight text-ink">
            {selectedProvider ? selectedProvider.name : providerTemplateById(form.templateId).label}
          </p>
          <p className="mt-2 max-w-xs text-sm leading-6 text-ink-muted">
            {selectedProvider
              ? `${selectedProviderReadiness?.label || "Selected"} provider open for editing and deeper runtime details.`
              : "Compose a new provider from the selected template, then validate and save it into the registry."}
          </p>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 gap-4 px-5 py-5 md:grid-cols-2 xl:grid-cols-3">
                {groupProviders.map((provider) => {
                  ...
                  <article
                    ...
                    className={[
                      "panel provider-card group relative cursor-pointer overflow-hidden border-white/8 transition-all duration-300",
                      "hover:-translate-y-0.5 hover:border-accent/30 hover:shadow-[0_24px_60px_-36px_rgba(15,23,42,0.95)]",
                      selectedProviderId === provider.id ? "provider-card--selected" : "",
                      selectedProviderId === provider.id
                        ? "border-accent/40 ring-1 ring-accent/20 shadow-[0_24px_80px_-40px_rgba(34,211,238,0.35)]"
                        : "",
                    ].join(" ")}
                    title={formatProviderHoverDetails(provider)}
                  >
                    <div className={`absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-current to-transparent opacity-70 ${tone.accent}`} />
                    <div className="panel-head flex items-center justify-between">
                      <div className="provider-card__header">
                        <div className="flex items-center gap-2">
                          {providerTypeIcon(provider.providerType as ProviderType)}
                          <span className="provider-card__eyebrow">{formatProviderTypeLabel(provider.providerType)}</span>
                        </div>
                        <span className="provider-card__title">{provider.name}</span>
                        <span className="provider-card__subtitle">{provider.id}</span>
                      </div>
                      <span className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] ${providerReadinessClasses(readiness.state)}`}>
                        {readiness.label}
                      </span>
                    </div>
                    <div className="panel-body space-y-4">
                      ...
                      <div className="provider-card__stats">
                        <div className="provider-card__stat">
                          ...
                          <span className="provider-card__stat-value">{formatProviderModelLabel(provider)}</span>
                        </div>
                        <div className="provider-card__stat">
                          <span className="provider-card__stat-label">Credential</span>
                          <span className="provider-card__stat-value">{formatProviderCredentialState(provider)}</span>
                        </div>
                        <div className="provider-card__stat">
                          <span className="provider-card__stat-label">Locality</span>
                          <span className="provider-card__stat-value">{formatProviderLocalityLabel(provider)}</span>
                        </div>
                      </div>
                      ...
                      {selectedProviderId === provider.id ? (
                        <div className="space-y-3 rounded-2xl border border-accent/25 bg-accent/5 px-4 py-4">
                          ...
                          <div className="grid gap-2 text-xs">
                            ...
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <span className="text-right text-ink">{formatProviderCredentialState(provider)}</span>
                            </div>
                        ...
                        <div className="provider-card__footer">
                          ...
                          <span className="provider-card__arrow" aria-hidden="true">&gt;</span>
                        </div>
                      )}
                    </div>
                  </article>
