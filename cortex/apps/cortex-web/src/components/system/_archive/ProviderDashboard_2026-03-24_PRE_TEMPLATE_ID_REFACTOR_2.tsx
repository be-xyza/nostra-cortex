    ...
    <div className="provider-dashboard animate-in fade-in duration-500 p-6">
      ...
      <div className="mb-8 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
          ...
          <div className="panel-body grid gap-5">
            ...
            <details className="rounded-2xl border border-border-subtle bg-surface-elevated px-4 py-3">
              ...
              <p className="mt-2 text-xs leading-5 text-ink-faint">
                Hidden routing, credential, discovery, and metadata settings for the current provider.
              </p>
              <div className="mt-4 grid gap-4">
                {selectedProvider ? (
                  <div className="rounded-2xl border border-dashed border-border-strong px-4 py-4 text-sm text-ink-muted">
                    Selected provider: {selectedProvider.name}. Use the menu below to edit the record, credentials, and discovery state.
                  </div>
                ) : (
                  <div className="rounded-2xl border border-dashed border-border-strong px-4 py-6 text-sm text-ink-muted">
                    Select a provider card to edit its hidden routing, credential, and discovery settings.
                  </div>
                )}
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <p className="text-xs leading-5 text-ink-faint">
                      This is the registry key for the provider record. New providers are seeded from the template id
                      and receive a numeric suffix when the same template is used more than once.
                    </p>
                  </label>
                  <label className="grid gap-2 text-sm">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <p className="text-xs leading-5 text-ink-faint">
                      {modelSelectOptions.length > 0
                        ? "Type to filter or use the browser suggestions to pick a discovered model."
                        : adapterStatusError
                          ? `Model discovery is currently limited: ${adapterStatusError}`
                      : "Validate the provider or refresh local models to populate the dropdown."}
                    </p>
                  </label>
                </div>
                ...
                <div className="rounded-xl border border-border-subtle bg-surface-elevated px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-ink">Usage links</p>
                      <p className="mt-1 text-xs leading-5 text-ink-faint">
                        These fields control how this provider is referenced by routing and credential usage.
                      </p>
                    </div>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    <label className="grid gap-2 text-sm">
                      ...
                      />
                      <p className="text-xs leading-5 text-ink-faint">
                        Used when this provider is bound into space routing or adapter-aware flows.
                      </p>
                    </label>
                    <label className="grid gap-2 text-sm">
                      ...
                      <p className="text-xs leading-5 text-ink-faint">
                        {visibleCredentialBindings.length > 0
                          ? "Choose the credential record this provider should use."
                          : "No provider-linked credentials are registered yet."}
                      </p>
                    </label>
                  </div>
                </div>
                ...
                <details className="rounded-xl border border-border-subtle bg-surface-elevated px-4 py-3">
                  ...
                  <div className="mt-4 grid gap-4">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div className="space-y-2">
                        ...
                        <p className="max-w-xl text-xs leading-5 text-ink-muted">
                          Discovery is adapter-bound and runs in the background. It updates the available model catalog
                          for the dropdown below, but it does not create or save a provider until you explicitly start one.
                        </p>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Base URL</p>
                        <p className="mt-2 break-all text-sm text-ink">{discoveryBaseUrl}</p>
                      </div>
                      <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Models</p>
                        <p className="mt-2 text-sm text-ink">
                          {discoveryModelOptions.length > 0 ? `${discoveryModelOptions.length} available` : "No catalog yet"}
                        </p>
                      </div>
                      <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Status</p>
                        <p className="mt-2 text-sm text-ink">{adapterStatus?.enabled ? "Connected" : "Idle"}</p>
                      </div>
                    </div>
                    ...
                    {discoveryModelOptions.length > 0 ? (
                      <details className="rounded-xl border border-border-subtle/70 bg-black/8 px-3 py-3">
                        <summary className="cursor-pointer select-none text-xs font-semibold uppercase tracking-[0.18em] text-ink-muted">
                          View models
                        </summary>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <span className="text-xs text-ink-faint">
                        The model dropdown stays in sync with the latest catalog.
                      </span>
                    </div>
                  </div>
                </details>
                ...
                <details className="rounded-xl border border-border-subtle bg-surface-elevated px-4 py-3">
                  ...
                  <p className="mt-2 text-xs leading-5 text-ink-faint">
                    Optional JSON metadata for the provider record.
                  </p>
                  <textarea
                ...
                {visibleCredentialBindings.length > 0 ? (
                  <details className="rounded-xl border border-border-subtle bg-surface-elevated px-4 py-3">
                    ...
                    <div className="mt-3 grid gap-3">
                      {visibleCredentialBindings.map((binding) => (
                        <div key={binding.credentialBindingId} className="rounded-xl border border-border-subtle bg-black/8 px-4 py-4">
        ...
        <details className="panel">
          <summary className="panel-head flex cursor-pointer list-none items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Sparkles className="h-4 w-4" />
              Selected provider menu
            </div>
            {selectedProvider ? (
              <span className="rounded-full border border-border-subtle px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-muted">
          ...
          <div className="panel-body grid gap-4">
                {selectedProvider ? (
                  <div className="rounded-2xl border border-border-subtle bg-surface-elevated/70 px-4 py-4">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink-faint">Usage summary</p>
                        <p className="mt-1 text-sm font-semibold text-ink">{selectedProvider.name}</p>
                        <p className="mt-1 text-xs text-ink-muted">
                          {formatProviderLocalityLabel(selectedProvider)} · {selectedProvider.providerType}
                        </p>
                      </div>
                      <div className="rounded-xl border border-border-subtle bg-black/10 px-3 py-2 text-right">
                        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Linked credential</p>
                        <p className="mt-1 text-xs text-ink">
                          {selectedProviderCredentialBinding?.label || selectedProvider.credentialBindingId || "System default"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Endpoint</p>
                      <p className="mt-2 break-all text-sm text-ink">{selectedProvider.endpoint}</p>
                    </div>
                    <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Default model</p>
                      <p className="mt-2 text-sm text-ink">{formatProviderModelLabel(selectedProvider)}</p>
                    </div>
                    <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Adapter set</p>
                      <p className="mt-2 text-sm text-ink">{selectedProvider.adapterSetRef || "Not bound"}</p>
                    </div>
                    <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink-faint">Locality</p>
                      <p className="mt-2 text-sm text-ink">{formatProviderLocalityLabel(selectedProvider)}</p>
                    </div>
                    </div>

                    <div className="mt-4 flex flex-wrap gap-2 text-[11px] text-ink-muted">
                      <span className="rounded-full border border-border-subtle px-2 py-1">
                        {selectedProvider.hasCredential ? "Credential linked" : "No credential linked"}
                      </span>
                      <span className="rounded-full border border-border-subtle px-2 py-1">
                        {selectedProvider.isActive ? "Enabled" : "Disabled"}
                      </span>
                      {selectedProvider.batchPolicy ? (
                      <span className="rounded-full border border-border-subtle px-2 py-1">
                        {formatProviderBatchSummary(selectedProvider.batchPolicy)}
                      </span>
                    ) : null}
                  </div>

                  {(selectedProvider.supportedModels ?? []).length > 0 ? (
                    <details className="mt-4 rounded-xl border border-border-subtle bg-black/8 px-3 py-3">
                      <summary className="cursor-pointer select-none text-xs font-semibold uppercase tracking-[0.18em] text-ink-muted">
                        View models
                      </summary>
                      <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <details className="mt-4 rounded-xl border border-border-subtle bg-black/8 px-3 py-3">
                      ...
                      <div className="mt-3 grid gap-1 text-xs text-ink-muted">
                        <div className="flex justify-between gap-3">
                          <span>Family</span>
                          <span className="text-ink opacity-80">{selectedProvider.topology.familyId}</span>
                        </div>
                        {selectedProvider.topology.profileId ? (
                          <div className="flex justify-between gap-3">
                            <span>Profile</span>
                            <span className="text-ink opacity-80">{selectedProvider.topology.profileId}</span>
                          </div>
                        ) : null}
                        <div className="flex justify-between gap-3">
                          <span>Instance</span>
                          <span className="text-ink opacity-80">{selectedProvider.topology.instanceId}</span>
                        </div>
                        {selectedProvider.topology.discoverySource ? (
                          <div className="flex justify-between gap-3">
                            <span>Source</span>
                            <span className="text-ink opacity-80">{selectedProvider.topology.discoverySource}</span>
                          </div>
                        ) : null}
                      </div>
                    </details>
              ...
              <div className="rounded-xl border border-dashed border-border-strong px-4 py-6 text-center text-sm text-ink-muted">
                {selectedProvider
                  ? `Credentials linked to ${selectedProvider.name} are listed below.`
                  : "Select a provider card to inspect its usage details and linked credentials."}
              </div>

                  {visibleCredentialBindings.length > 0 ? (
                    <div className="grid gap-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink-faint">Linked credentials</p>
                      {visibleCredentialBindings.map((binding) => (
                ...
                <div className="rounded-xl border border-dashed border-border-strong px-4 py-8 text-center text-sm text-ink-muted">
                  No credentials are registered for {selectedProvider.name} yet.
                </div>
              ) : null}
            </div>
          </details>
                  ...
                  >
                    ...
                    <div className="panel-body space-y-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className="rounded-full border border-border-subtle px-2 py-0.5 text-[10px] font-medium text-ink-muted">
                          {provider.batchPolicy ? formatProviderBatchSummary(provider.batchPolicy) : "No batch policy"}
                        </span>
                      </div>

                      <p className="pt-1 text-[10px] text-ink-faint">
                        {selectedProviderId === provider.id ? "Open details to edit" : "Click to preview or edit"}
                      </p>
                    </div>
                  </article>
                ))}
