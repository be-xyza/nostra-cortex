...
export function getProviderOperationalReadiness(
  provider: Pick<
    ProviderRecord,
    "providerType" | "llmType" | "isActive" | "hasCredential" | "credentialBindingId" | "supportedModels" | "adapterHealthError" | "upstreamModelsError" | "endpoint" | "topology"
  >,
  ...
  const isAnonymousLocalRuntime =
    ...
    && !provider.credentialBindingId?.trim()
    && (
  ...
  if (provider.hasCredential && catalogSize > 0) {
    return {
      state: "ready",
      label: "Ready",
      detail: `Credential is available and ${catalogSize} model${catalogSize === 1 ? "" : "s"} are loaded.`,
      issues: [],
    };
  }
  ...
  if (isAnonymousLocalRuntime) {
    return {
      state: "ready",
      label: "Ready",
      detail: `Local runtime discovery surfaced ${catalogSize} model${catalogSize === 1 ? "" : "s"} without a provider-specific credential.`,
      issues: [],
    };
  }
  ...
  if (provider.hasCredential) {
    return {
      state: "neutral",
      label: "Credential linked",
      detail: "Credential is available. Refresh the catalog to confirm the runtime and models.",
      issues: [],
    };
  }
