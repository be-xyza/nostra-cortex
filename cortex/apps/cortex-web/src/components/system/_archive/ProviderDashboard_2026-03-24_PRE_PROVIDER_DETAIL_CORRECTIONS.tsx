...
export const ProviderDashboard: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const supportedModels = probeResult?.supportedModels
      ...
      setSubmitMessage(
        form.apiKey.trim()
          ...
          : "Provider saved. Add a credential later if this runtime needs one.",
      );
    } catch (err) {
  ...
  const sheetSubtitle = isCreateSheet
    ? "Compose a provider record without crowding the registry."
    : selectedProviderReadiness?.detail ?? "Review runtime details, credentials, and models.";
    ...
    <div className="provider-dashboard min-h-full bg-cortex-surface-base">
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-white/62">
                  Scan provider readiness, open deep detail in a side sheet, and treat discovery as a global runtime utility rather than inline page clutter.
                </p>
      ...
      {isProviderSheetOpen ? (
        <>
          ...
          />
          <aside className="fixed inset-y-0 right-0 z-[120] flex w-full max-w-[46rem] flex-col border-l border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(2,6,23,0.97))] shadow-[0_32px_120px_-48px_rgba(0,0,0,0.85)]">
                  ...
                  <div className="space-y-5">
                    <section className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-white/60">Choose the provider shape for this record.</p>
                        </div>
                    ...
                    {selectedProvider ? (
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-4">
                          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-white/38">Topology</p>
                          <p className="mt-2 text-sm leading-6 text-white/62" title={formatProviderTopologySummary(selectedProvider)}>
                            {formatProviderTopologySummary(selectedProvider)}
                          </p>
                        </div>
                      </div>
                    ) : null}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <label className="grid gap-2 text-sm">
                        ...
                        <input
                          ...
                          type="password"
                          placeholder="Leave blank to save without creating a credential"
                          className="rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-white outline-none placeholder:text-white/26"
                        />
                      </label>
                    </div>
      ...
      {panelState.kind === "discovery" ? (
        <>
          <button
            ...
            className="fixed inset-0 z-[110] bg-slate-950/55 backdrop-blur-sm"
          />
          <aside className="fixed inset-y-0 right-0 z-[120] flex w-full max-w-[40rem] flex-col border-l border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(2,6,23,0.97))] shadow-[0_32px_120px_-48px_rgba(0,0,0,0.85)]">
            <div className="border-b border-white/8 px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
