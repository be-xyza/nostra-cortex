        ...
        <div className={["relative z-10 px-4 pb-10 pt-4 transition-[padding] duration-300 md:px-6", chromeState.contentPaddingClass].join(" ")}>
          <header
            className={[
              ...
              chromeState.compactRegistryChrome ? "mb-4 rounded-[1.5rem] px-4 py-3 md:px-4" : "mb-6 rounded-[2rem] px-4 py-4 md:px-5",
            ].join(" ")}
          >
            <div className={chromeState.headerLayoutClass}>
              <div className="min-w-0">
                ...
                <div className={["mt-3 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.22em] text-white/45", chromeState.compactRegistryChrome ? "gap-y-1.5" : ""].join(" ")}>
              ...
              <div className={["flex gap-3", chromeState.compactRegistryChrome ? "flex-wrap items-center" : "flex-col md:flex-row md:items-center"].join(" ")}>
                <label className={["flex items-center gap-3 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm text-white/70 shadow-inner shadow-black/10", chromeState.compactRegistryChrome ? "min-w-[14rem] flex-1" : "min-w-[18rem]"].join(" ")}>
                ...
                <div className="flex flex-wrap items-center gap-2 rounded-full border border-white/8 bg-white/[0.03] p-1.5 shadow-sm backdrop-blur-sm">
                  <button
                    type="button"
              ...
              return (
                <section
                  ...
                  className={[
                    ...
                    chromeState.compactRegistryRows ? "rounded-[1.35rem]" : "rounded-[1.75rem]",
                  ].join(" ")}
                >
                  <div className={["flex items-center justify-between gap-3 border-b border-white/8", chromeState.compactRegistryRows ? "px-3 py-3" : "px-4 py-4"].join(" ")}>
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center justify-center border ${tone.border} bg-white/[0.03] ${tone.accent} ${chromeState.compactRegistryRows ? "h-9 w-9 rounded-xl" : "h-10 w-10 rounded-2xl"}`}>
                    ...
                    {sectionProviders.map((provider) => {
                      ...
                      return (
                        <button
                          ...
                          className={[
                            "grid w-full text-left transition hover:bg-white/[0.035]",
                            chromeState.compactRegistryRows
                              ? "gap-3 px-3 py-3 lg:grid-cols-[minmax(0,1.8fr)_auto]"
                              : "gap-4 px-4 py-4 md:grid-cols-[minmax(0,2.2fr)_minmax(0,1fr)_auto]",
                          ].join(" ")}
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <p className={`${chromeState.compactRegistryRows ? "text-sm" : "text-base"} truncate font-semibold text-white/92`}>{provider.name}</p>
                            ...
                            {chromeState.compactRegistryRows ? (
                              <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-white/68">
                                <span>{formatProviderModelLabel(provider)}</span>
                                <span className="text-white/28">•</span>
                                <span>{formatProviderCredentialState(provider)}</span>
                                <span className="text-white/28">•</span>
                                <span>{formatProviderLocalityLabel(provider)}</span>
                              </div>
                            ) : null}
                          </div>
                          ...
                          <div className={["flex gap-3", chromeState.compactRegistryRows ? "items-center justify-end" : "items-center justify-between md:flex-col md:items-end md:justify-center"].join(" ")}>
                            <div className={`flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.16em] text-white/42 ${chromeState.compactRegistryRows ? "justify-end" : "md:justify-end"}`}>
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      <label className="grid gap-2 text-sm">
                        ...
                        {defaultModelControlState.control === "select" ? (
                          <select
                          ...
                          >
                            <option value="">{providerTemplateById(form.templateId).defaultModelHint}</option>
