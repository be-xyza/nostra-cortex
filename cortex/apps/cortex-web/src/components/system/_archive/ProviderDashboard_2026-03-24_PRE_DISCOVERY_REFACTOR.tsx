      ...
      <header className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-border-subtle bg-surface-elevated px-4 py-2 text-sm font-semibold transition-all hover:bg-opacity-80 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Sparkles className={`h-4 w-4 ${status === "discovering" || isRefreshingAdapterModels ? "animate-pulse" : ""}`} />
              ...
              <div className="rounded-2xl border border-border-subtle bg-surface-elevated/70 px-4 py-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="space-y-2">
                    <p className="text-sm font-semibold text-ink">Refresh the adapter catalog, then create a provider from it.</p>
                    <p className="max-w-md text-xs leading-5 text-ink-muted">
                      Discovery is limited to the configured adapter runtime. It surfaces what the adapter exposes,
                      not every AI app installed on the machine.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-border-subtle/80 bg-black/10 px-3 py-3">
                    ...
                    <p className="mt-2 text-sm text-ink">
                      {adapterStatus?.enabled ? "Connected" : "Idle"}
                    </p>
                  </div>
                </div>

                <p className="mt-4 text-xs leading-5 text-ink-muted">{discoveryCatalogDescription}</p>
                {adapterStatusError ? (
                  <p className="mt-2 text-xs text-amber-200">
                    Discovery note: {adapterStatusError}
                  </p>
                ) : null}

                {discoveryModelOptions.length > 0 ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {discoveryModelOptions.slice(0, 6).map((model) => (
                      <button
                        key={model}
                        type="button"
                    ...
                    {discoveryModelOptions.length > 6 ? (
                      <span className="rounded-full border border-border-subtle px-3 py-1 text-[11px] font-medium text-ink-muted">
                        +{discoveryModelOptions.length - 6} more
                      </span>
                    ) : null}
                  </div>
                ) : null}
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-xs text-ink-faint">
                    The model dropdown below stays in sync with the catalog.
                  </span>
                </div>
              </div>
            </div>
