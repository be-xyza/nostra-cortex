...
export const workbenchApi = {
  ...
  createSystemProviderCredential: (payload: {
    providerId?: string;
    label: string;
    apiKey: string;
    metadata?: Record<string, string>;
  }) =>
    request<ProviderCredentialBindingRecord>("/api/system/providers/credentials", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  updateSystemProviderCredential: (
    ...
    request<ProviderCredentialBindingRecord>(
      `/api/system/providers/credentials/${encodeURIComponent(credentialBindingId)}`,
      {
        method: "PUT",
        body: JSON.stringify(payload),
      },
