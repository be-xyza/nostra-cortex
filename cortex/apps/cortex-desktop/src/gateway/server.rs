...
fn dpub_workspace_root() -> PathBuf {
    std::env::var("NOSTRA_DPUB_WORKSPACE_ROOT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| workspace_root())
}

fn dpub_graph_workspace_dir() -> PathBuf {
    dpub_workspace_root()
        .join("research")
        .join("000-initiative-graph")
}

fn dpub_graph_path() -> PathBuf {
    dpub_graph_workspace_dir().join("initiative_graph.json")
}

fn dpub_path_assessment_path() -> PathBuf {
    dpub_graph_workspace_dir().join("path_assessment.json")
}

fn dpub_doctor_path() -> PathBuf {
    dpub_graph_workspace_dir().join("doctor_report.json")
}

fn dpub_simulations_dir() -> PathBuf {
    dpub_graph_workspace_dir().join("simulations")
}

fn dpub_editions_dir() -> PathBuf {
    dpub_graph_workspace_dir().join("editions")
}

fn dpub_run_log_dir() -> PathBuf {
    std::env::var("NOSTRA_DPUB_RUN_LOG_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| workspace_logs_dir().join("initiative_graph").join("runs"))
}

fn dpub_steward_packet_dir() -> PathBuf {
...
fn dpub_load_graph() -> Result<InitiativeGraphV1, axum::response::Response> {
    dpub_read_json(&dpub_graph_path())
}

fn dpub_load_path_bundle() -> Result<PathAssessmentBundleV1, axum::response::Response> {
    dpub_read_json(&dpub_path_assessment_path())
}

fn dpub_load_doctor() -> Result<DoctorReport, axum::response::Response> {
    dpub_read_json(&dpub_doctor_path())
}
...
fn siq_load_projection_optional() -> Option<SiqGraphProjection> {
    let raw = fs::read_to_string(siq_graph_projection_path()).ok()?;
    let mut projection = serde_json::from_str::<SiqGraphProjection>(&raw).ok()?;
    normalize_siq_projection(&mut projection);
    Some(projection)
}

fn siq_load_gate_summary_optional() -> Option<SiqGateSummary> {
    let raw = fs::read_to_string(siq_gate_summary_path()).ok()?;
...
fn dpub_read_run_records() -> Vec<DpubRunRecord> {
    let mut out = Vec::new();
    let dir = dpub_run_log_dir();
...
fn dpub_persist_run_record(record: &DpubRunRecord) -> Result<(), String> {
    let path = dpub_run_log_dir().join(format!("{}.json", record.run_id));
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|err| err.to_string())?;
    }
...
fn dpub_edition_entries() -> Vec<DpubEditionEntry> {
    let mut out = Vec::new();
    let dir = dpub_editions_dir();
    let entries = match fs::read_dir(&dir) {
        Ok(entries) => entries,
        Err(_) => return out,
    };
...
fn dpub_edition_trends(goal: &str, window: usize) -> Result<DpubEditionTrendResponse, String> {
    let mut entries = dpub_edition_entries();
    ...
    for entry in entries {
        let snapshot_path = dpub_editions_dir()
            .join(&entry.version)
            .join("snapshot.json");
...
fn siq_graph_projection_path() -> PathBuf {
...
async fn get_siq_gates_latest() -> impl IntoResponse {
    let path = siq_gate_summary_path();
...
async fn get_siq_graph_projection() -> impl IntoResponse {
    let path = siq_graph_projection_path();
...
async fn get_siq_health() -> impl IntoResponse {
    ...
    let gate_path = siq_gate_summary_path();
    let projection_path = siq_graph_projection_path();
    let runs_dir = siq_runs_dir();
    ...
    let execution: Result<(), String> = (|| {
        ...
        let run_ingest = |phase_results: &mut Vec<DpubPhaseResult>,
         ...
         -> Result<(), String> {
            ...
            if let Some(projection) = siq_load_projection_optional() {
                let gate_summary = siq_load_gate_summary_optional();
                artifacts.insert(
...
async fn get_initiative_graph_overview() -> impl IntoResponse {
    let graph = dpub_load_graph().ok();
    let path_bundle = dpub_load_path_bundle().ok();
    let runs = dpub_read_run_records();
    let latest_run_summary = runs.first().map(dpub_run_history_item);
    let siq_projection = siq_load_projection_optional();
    let siq_summary = siq_load_gate_summary_optional();

    let health = json!({
        "status": if graph.is_some() { "ok" } else { "missing" },
        "workspaceRoot": dpub_workspace_root().display().to_string(),
        "graphPath": dpub_graph_path().display().to_string(),
        "pathAssessmentPath": dpub_path_assessment_path().display().to_string(),
        "doctorPath": dpub_doctor_path().display().to_string(),
        "doctorExists": dpub_doctor_path().exists(),
        "simulationsDir": dpub_simulations_dir().display().to_string(),
        "editionsDir": dpub_editions_dir().display().to_string(),
        "runLogDir": dpub_run_log_dir().display().to_string()
    });
    ...
    let overview = DpubWorkbenchOverview {
        ...
        artifact_paths: json!({
            "graph": dpub_graph_path().display().to_string(),
            "pathAssessment": dpub_path_assessment_path().display().to_string(),
            "doctor": dpub_doctor_path().display().to_string(),
            "simulationsDir": dpub_simulations_dir().display().to_string(),
            "editionsDir": dpub_editions_dir().display().to_string(),
            "siqProjection": siq_graph_projection_path().display().to_string(),
            "siqGateSummary": siq_gate_summary_path().display().to_string()
        }),
    };
    Json(overview).into_response()
}

async fn get_initiative_graph_graph() -> impl IntoResponse {
    match dpub_load_graph() {
        Ok(graph) => Json(graph).into_response(),
        Err(err) => err,
    }
}

async fn get_initiative_graph_path_assessment() -> impl IntoResponse {
    match dpub_load_path_bundle() {
        Ok(bundle) => Json(bundle).into_response(),
        Err(err) => err,
    }
}

async fn get_initiative_graph_lens_summary() -> impl IntoResponse {
    match dpub_load_graph() {
...
async fn get_initiative_graph_edition_trends(
    ...
    match dpub_edition_trends(&goal, window) {
...
async fn get_initiative_graph_doctor() -> impl IntoResponse {
    match dpub_load_doctor() {
        Ok(report) => Json(report).into_response(),
        Err(err) => err,
    }
}

async fn get_initiative_graph_simulations() -> impl IntoResponse {
    let dir = dpub_simulations_dir();
    if !dir.exists() {
...
async fn get_initiative_graph_editions() -> impl IntoResponse {
    Json(dpub_edition_entries()).into_response()
}
...
async fn get_initiative_graph_edition_diff(
    ...
    match diff_editions(&dpub_workspace_root(), &query.from, &query.to) {
...
async fn get_initiative_graph_runs(Query(query): Query<DpubRunHistoryQuery>) -> impl IntoResponse {
    let mut items = dpub_read_run_records()
        .iter()
...
async fn get_initiative_graph_run(Path(run_id): Path<String>) -> impl IntoResponse {
    ...
    let path = dpub_run_log_dir().join(format!("{}.json", run_id));
...
async fn post_initiative_graph_pipeline_query(
    ...
    match query_graph(&dpub_workspace_root(), &payload.kind, &payload.id) {
        Ok(result) => Json(result).into_response(),
        Err(err) => dpub_error(
            StatusCode::BAD_REQUEST,
            "DPUB_QUERY_FAILED",
...
async fn post_initiative_graph_lens_evaluate(
    ...
    let graph = match dpub_load_graph() {
        Ok(graph) => graph,
        Err(err) => return err,
    };
...
async fn get_initiative_graph_violations_by_node() -> impl IntoResponse {
    let graph = match dpub_load_graph() {
        Ok(graph) => graph,
        Err(err) => return err,
    };
...
async fn post_initiative_graph_steward_packet_export(
    ...
    let path_bundle = match dpub_load_path_bundle() {
        Ok(bundle) => bundle,
        Err(err) => return err,
    };

    let editions = dpub_edition_entries();
    ...
    let diff = match diff_editions(&dpub_workspace_root(), &from_version, &to_version) {
        Ok(diff) => diff,
        Err(err) => {
            return dpub_error(
                StatusCode::BAD_REQUEST,
    ...
    let packet_path = dpub_steward_packet_dir().join(file_name);
...
async fn get_system_build() -> Json<SystemBuild> {
    ...
    Json(SystemBuild {
        ...
        gateway_port,
        workspace_root: dpub_workspace_root().display().to_string(),
    })
}
