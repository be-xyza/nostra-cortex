...
fn endpoint_inventory_json_mirrors_tsv() {
    ...
    assert_eq!(
        tsv, json,
        "endpoint_inventory.json must mirror endpoint_inventory.tsv"
    );
}
...
fn gateway_inventory_covers_required_endpoint_classes() {
    ...
    for class in classes.classes {
        let has_match = paths
            .iter()
            .any(|path| path.starts_with(&class.path_prefix));
        assert!(
            has_match,
            "missing endpoint coverage for class '{}' ({})",
            class.name, class.path_prefix
        );
    }
}
...
fn gateway_parity_replay_cases_match_between_legacy_and_runtime() {
    ...
    for case_path in case_paths {
        ...
        let endpoint = (
            case.request.method.to_uppercase(),
            case.request.path.clone(),
        );
        assert!(
...
fn assert_case_parity(case: &ParityCase) {
    ...
    match mode {
        ...
        "subset" => {
            ...
            for key in &case.normalization.required_keys {
                let left = lookup_path(&legacy, key).unwrap_or_else(|| {
                    panic!(
                        "missing required key '{}' in legacy response for '{}'",
                        key, case.case_id
                    )
                });
                let right = lookup_path(&runtime, key).unwrap_or_else(|| {
                    panic!(
                        "missing required key '{}' in runtime response for '{}'",
                        key, case.case_id
                    )
                });
                assert_eq!(
        ...
        other => panic!(
            "unsupported normalization mode '{}' in case '{}'",
            other, case.case_id
        ),
    }
}
