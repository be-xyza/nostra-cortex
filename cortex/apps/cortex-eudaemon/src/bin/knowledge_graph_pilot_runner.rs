...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let baseline_path = workspace_root.join("logs/knowledge/retrieval_benchmark_latest.json");
    let (baseline, baseline_source_ref): (LegacyRetrievalBenchmarkReport, &str) =
        if baseline_path.exists() {
            ...
            (
                load_json(&fixture_dir.join("legacy_037_retrieval_benchmark_latest.json"))?,
                "nostra://fixtures/knowledge/legacy_037_retrieval_benchmark_latest.json",
            )
        };
