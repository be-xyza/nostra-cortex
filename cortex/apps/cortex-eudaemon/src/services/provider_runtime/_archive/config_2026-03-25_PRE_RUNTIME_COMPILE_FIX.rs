...
pub fn migrate_provider_registry_state(mut state: ProviderRegistryState) -> ProviderRegistryState {
    ...
    for credential in &mut state.credentials {
        credential.provider_id = credential.provider_id.trim().to_string();
        if let Some(provider) = provider_lookup.get(&credential.provider_id) {
            ...
            if let Some(profile_id) = context.profile_id.as_ref() {
                credential
                    .metadata
                    .insert("profileId".to_string(), profile_id.clone());
            }
            credential
                .metadata
            ...
            if let Some(discovery_source) = context.discovery_source.as_ref() {
                credential
                    .metadata
                    .insert("discoverySource".to_string(), discovery_source.clone());
            }
        }
    }
    ...
    state.adapter_bindings.retain(|binding| {
...
mod tests {
    ...
    fn migrate_provider_registry_state_normalizes_real_provider_records_and_promotes_catalogs_to_discovery() {
        ...
        assert_eq!(
            migrated.credentials[0].metadata.get("providerKind"),
