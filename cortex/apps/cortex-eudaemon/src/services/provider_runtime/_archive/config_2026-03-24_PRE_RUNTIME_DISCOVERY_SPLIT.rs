...
pub const LEGACY_PROVIDER_ID: &str = "llm_adapter";
...
pub struct ProviderRegistryState {
    ...
    #[serde(default)]
    pub adapter_bindings: Vec<AdapterBindingRecord>,
    #[serde(default)]
    ...
    ProviderDiscoveryRecord {
        ...
        topology: Some(provider_runtime_context_from_resolved_state(
            &ResolvedProviderRuntimeState {
                ...
                credential_binding_id: settings.credential_binding_id.clone(),
                has_credential: settings.credential_binding_id.is_some(),
...
pub fn normalize_provider_id_reference(provider_id: &str) -> Option<String> {
...
pub fn migrate_provider_registry_state(mut state: ProviderRegistryState) -> ProviderRegistryState {
    let mut used_provider_ids = state
        .providers
        .iter()
        .filter(|provider| provider.provider_id != LEGACY_PROVIDER_ID)
        .map(|provider| provider.provider_id.clone())
        .collect::<std::collections::BTreeSet<_>>();
    let mut legacy_rewrites = std::collections::BTreeMap::new();
    ...
    for mut provider in state.providers.drain(..) {
        ...
        if provider.provider_id == LEGACY_PROVIDER_ID {
            let mut candidate_id =
                derive_real_provider_id(&provider_kind, &provider_type, locality_kind);
            let mut suffix = 2usize;
            while used_provider_ids.contains(&candidate_id) {
                candidate_id = format!(
                    "{}_{}",
                    derive_real_provider_id(&provider_kind, &provider_type, locality_kind),
                    suffix
            ...
            used_provider_ids.insert(candidate_id.clone());
            legacy_rewrites.insert(provider.provider_id.clone(), candidate_id.clone());
            provider.provider_id = candidate_id;
            if provider.name.trim().is_empty()
                || provider.name.trim().eq_ignore_ascii_case("primary llm adapter")
            {
                provider.name = default_provider_name(&provider_kind, &provider_type, locality_kind);
            }
            provider.adapter_set_ref = None;
        } else {
            used_provider_ids.insert(provider.provider_id.clone());
            if provider.name.trim().is_empty() {
                provider.name = default_provider_name(&provider_kind, &provider_type, locality_kind);
            }
        }
    ...
    for credential in &mut state.credentials {
        if credential.provider_id == LEGACY_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(LEGACY_PROVIDER_ID) {
                credential.provider_id = next_provider_id.clone();
                credential
                    .metadata
                    .insert("providerId".to_string(), next_provider_id.clone());
            }
        }
    }

    for record in &mut discovery {
        if record.provider_id == LEGACY_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(LEGACY_PROVIDER_ID) {
                record.provider_id = next_provider_id.clone();
            }
        }
    }

    for binding in &mut state.adapter_bindings {
        if binding.bound_provider_id == LEGACY_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(LEGACY_PROVIDER_ID) {
                binding.bound_provider_id = next_provider_id.clone();
            }
        }
    }
    ...
    {
        if let Some(bound_provider_id) = legacy_rewrites
        ...
        {
            state.adapter_bindings.push(AdapterBindingRecord {
...
mod tests {
    ...
    #[test]
    fn migrate_provider_registry_state_promotes_legacy_llm_adapter_into_binding_and_discovery() {
        let legacy_provider = ProviderRuntimeSettings {
            ...
            credential_binding_id: Some("cred_llm_adapter".to_string()),
            provider_family_id: Some("openrouter".to_string()),
            profile_id: Some("openai/gpt-5.4".to_string()),
            instance_id: Some("llm_adapter__https_openrouter.ai_api".to_string()),
            device_id: None,
            environment_id: None,
        ...
        let legacy_credential = ProviderCredentialBinding {
            credential_binding_id: "cred_llm_adapter".to_string(),
            provider_id: LEGACY_PROVIDER_ID.to_string(),
            ...
            metadata: BTreeMap::from([
                ("providerId".to_string(), LEGACY_PROVIDER_ID.to_string()),
        ...
        let migrated = migrate_provider_registry_state(ProviderRegistryState {
            ...
            credentials: vec![legacy_credential],
            adapter_bindings: Vec::new(),
            discovery: Vec::new(),
        });
        ...
        assert_eq!(migrated.providers[0].adapter_set_ref, None);
        assert_eq!(migrated.credentials[0].provider_id, "openrouter_primary");
        assert_eq!(
            migrated.credentials[0].metadata.get("providerId"),
            Some(&"openrouter_primary".to_string())
        );
        assert_eq!(migrated.adapter_bindings.len(), 1);
        assert_eq!(migrated.adapter_bindings[0].binding_id, "llm.default");
        assert_eq!(migrated.adapter_bindings[0].bound_provider_id, "openrouter_primary");
