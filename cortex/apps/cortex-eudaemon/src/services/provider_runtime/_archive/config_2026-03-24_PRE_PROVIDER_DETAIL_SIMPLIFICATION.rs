...
pub fn resolve_provider_id_alias(
    state: &ProviderRegistryState,
    provider_id: &str,
) -> Option<String> {
    ...
    if trimmed == DEFAULT_PROVIDER_ID {
        return preferred_binding_provider_id(state, "Llm");
    }
...
pub fn migrate_provider_registry_state(mut state: ProviderRegistryState) -> ProviderRegistryState {
    let mut used_provider_ids = state
        .providers
        .iter()
        .filter(|provider| provider.provider_id != DEFAULT_PROVIDER_ID)
    ...
    for credential in &mut state.credentials {
        if credential.provider_id == DEFAULT_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(DEFAULT_PROVIDER_ID) {
    ...
    for record in &mut discovery {
        if record.provider_id == DEFAULT_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(DEFAULT_PROVIDER_ID) {
    ...
    for binding in &mut state.adapter_bindings {
        if binding.bound_provider_id == DEFAULT_PROVIDER_ID {
            if let Some(next_provider_id) = legacy_rewrites.get(DEFAULT_PROVIDER_ID) {
...
mod tests {
    ...
    fn migrate_provider_registry_state_promotes_legacy_llm_adapter_into_binding_and_discovery() {
        ...
        let legacy_credential = ProviderCredentialBinding {
            ...
            updated_at: Some("2026-03-24T00:00:00Z".to_string()),
            metadata: BTreeMap::from([
                ("providerId".to_string(), DEFAULT_PROVIDER_ID.to_string()),
