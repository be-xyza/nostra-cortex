...
use crate::services::provider_runtime::responses_types::{CompletedResponse, ToolCallRequest};
...
pub async fn run_responses_tool_loop(
    ...
    for step in 0..max_steps {
        ...
        for call in completed.tool_calls.iter().cloned() {
            on_event(ProviderRuntimeStreamEvent::ToolCallReady(call.clone()));
    ...
    Err("provider_runtime_tool_loop_max_steps_exceeded".to_string())
}
...
mod tests {
    ...
    use crate::services::provider_runtime::config::{
        ProviderRuntimeConfig, ProviderRuntimeFailMode,
    };
    ...
    async fn tool_loop_executes_builtin_echo_and_completes() {
        ...
        let listener = match TcpListener::bind("127.0.0.1:0").await {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::PermissionDenied => {
                eprintln!(
                    "skipping provider runtime tool loop test: tcp bind denied ({})",
                    err
                );
                return;
            }
        ...
        let client = ProviderRuntimeClient::new(cfg).unwrap();
