...
use crate::services::provider_runtime::policy::{
    default_remote_discovery_enabled, preferred_executable_provider_for_type,
    runtime_endpoint_supports_execution,
};
...
fn build_runtime_context(
    ...
    let resolved_environment_id =
        environment_id.or_else(|| trimmed_env("CORTEX_PROVIDER_ENVIRONMENT_ID"));
    ...
    let resolved_discovery_source = discovery_source
        ...
        .or_else(|| {
            Some(
                if resolved_device_id.is_some() || resolved_environment_id.is_some() {
                    "environment"
                } else {
                    "heuristic"
                }
...
fn build_provider_batch_policy(
    ...
    let ordering_key = trimmed_env("CORTEX_PROVIDER_BATCH_ORDERING_KEY").or_else(|| {
        provider_kind
            .eq_ignore_ascii_case("doubleword")
            .then(|| "request_group_id".to_string())
    });
    let dedupe_key = trimmed_env("CORTEX_PROVIDER_BATCH_DEDUPE_KEY").or_else(|| {
        provider_kind
            .eq_ignore_ascii_case("doubleword")
            .then(|| "request_hash".to_string())
    });
    ...
    let timezone = trimmed_env("CORTEX_PROVIDER_BATCH_TIMEZONE").or_else(|| {
        provider_kind
            .eq_ignore_ascii_case("doubleword")
            .then(|| "UTC".to_string())
    });
...
pub fn build_provider_runtime_settings_from_env() -> ProviderRuntimeSettings {
    ...
    let batch_policy =
        build_provider_batch_policy(&provider_kind, &provider_id, &default_model, &base_url);
...
pub fn load_provider_registry_state() -> Result<ProviderRegistryState, String> {
    ...
    if !path.exists() {
        return Ok(migrate_provider_registry_state(
...
pub fn upsert_auth_binding(binding: AuthBindingRecord) -> Result<(), String> {
    let mut state = load_provider_registry_state()?;
    if let Some(existing) = state
        .auth_bindings
        .iter_mut()
...
fn provider_discovery_record_from_settings(
    ...
    let provider_kind = settings.provider_kind.clone().or_else(|| {
    ...
    ProviderDiscoveryRecord {
        ...
        topology: Some(provider_runtime_context_from_resolved_state(
            &ResolvedProviderRuntimeState {
                ...
                provider_kind: settings.provider_kind.clone().unwrap_or_else(|| {
                    infer_provider_kind(&settings.base_url, &settings.default_model)
                }),
                ...
                host_id: settings.host_id.clone().unwrap_or_else(|| {
                    default_runtime_host_id(
                        ...
                        settings
                            .locality_kind
                            .unwrap_or(ProviderLocalityKind::Cloud),
                        settings.environment_id.as_deref(),
                    )
                }),
                auth_binding_id: settings.auth_binding_id.clone(),
                auth_type: infer_auth_binding_type(settings.provider_kind.as_deref().unwrap_or("")),
...
fn upsert_runtime_host(hosts: &mut Vec<RuntimeHostRecord>, host: RuntimeHostRecord) {
...
fn runtime_host_from_provider(provider: &ProviderRuntimeSettings) -> RuntimeHostRecord {
    ...
    let host_id = provider.host_id.clone().unwrap_or_else(|| {
        default_runtime_host_id(
    ...
    let host_kind = infer_runtime_host_kind(
        &provider.base_url,
        locality_kind,
        provider.environment_id.as_deref(),
    );
    ...
    RuntimeHostRecord {
        ...
        remote_discovery_enabled: default_remote_discovery_enabled(&provider.base_url),
        execution_routable: runtime_endpoint_supports_execution(&provider.base_url),
        updated_at: provider.updated_at.clone(),
        metadata: BTreeMap::new(),
    }
}

fn default_auth_binding_for_provider(
    provider: &ProviderRuntimeSettings,
) -> Option<AuthBindingRecord> {
...
pub fn migrate_provider_registry_state(mut state: ProviderRegistryState) -> ProviderRegistryState {
    ...
    for host in &mut runtime_hosts {
        host.remote_discovery_enabled =
            host.remote_discovery_enabled || default_remote_discovery_enabled(&host.endpoint);
        host.execution_routable =
    ...
    for mut provider in state.providers.drain(..) {
        ...
        if provider
            ...
            .is_empty()
        {
            provider.auth_binding_id =
                default_auth_binding_for_provider(&provider).map(|binding| binding.auth_binding_id);
        }
    ...
    for binding in &mut state.auth_bindings {
        ...
        if let Some(provider) = provider_lookup.get(&binding.target_id) {
            ...
            if binding.auth_type != AuthBindingType::None {
                binding.auth_type =
                    infer_auth_binding_type(provider.provider_kind.as_deref().unwrap_or(""));
            }
    ...
    state.execution_bindings.retain(|binding| {
        !binding.binding_id.trim().is_empty()
            && provider_lookup.contains_key(&binding.bound_provider_id)
    });
...
pub fn resolve_provider_runtime_state() -> ResolvedProviderRuntimeState {
    ...
    let stored = loaded
        .as_ref()
        .and_then(|state| preferred_executable_provider_for_type(state, "Llm").cloned())
    ...
    let provider_kind = stored
        ...
        .or_else(|| {
            discovered
                .as_ref()
                .and_then(|item| item.provider_kind.clone())
        })
    ...
    let default_model = Some(stored.default_model.clone())
        ...
        .or_else(|| {
            discovered
                .as_ref()
                .and_then(|item| item.default_model.clone())
        })
        .or_else(|| {
    ...
    let locality_kind = discovered
        ...
        .unwrap_or_else(|| {
            infer_provider_locality_kind(&base_url, device_id.as_deref(), environment_id.as_deref())
        });
    ...
    let runtime_context = discovered
        ...
        .unwrap_or_else(|| {
            build_runtime_context(
                ...
                stored.updated_at.clone().unwrap_or_else(now_iso),
            )
        });
...
pub fn effective_provider_auth_source(binding: Option<&AuthBindingRecord>) -> Option<String> {
    binding
...
mod tests {
    ...
    fn provider_registry_state_round_trips_runtime_settings_and_auth_bindings() {
        ...
        assert_eq!(decoded.auth_bindings[0].source.as_deref(), Some("manual"));
    }
    ...
    fn migrate_provider_registry_state_normalizes_real_provider_records_and_promotes_catalogs_to_discovery()
     {
        ...
        assert_eq!(
            migrated.providers[0].adapter_set_ref.as_deref(),
            Some("adapter.default")
        );
        ...
        assert_eq!(
            migrated.execution_bindings[0].bound_provider_id,
            "openrouter_primary"
        );
    ...
    fn migrate_provider_registry_state_creates_runtime_hosts_auth_bindings_and_default_execution_binding()
     {
        ...
        assert!(
            migrated
                .runtime_hosts
                .iter()
                .any(|host| host.host_id == "host.local.primary"
                    && host.host_kind == RuntimeHostKind::Local)
        );
        assert!(
            migrated
                .runtime_hosts
        ...
        assert!(
            migrated
                .auth_bindings
                .iter()
                .any(|binding| binding.target_id == "ollama_local"
                    && binding.auth_type == AuthBindingType::None)
        );
        assert_eq!(migrated.execution_bindings.len(), 1);
        assert_eq!(
            migrated.execution_bindings[0].binding_id,
        ...
        assert_eq!(
            migrated
                .providers
                .iter()
                .find(|provider| provider.provider_id == "openrouter_primary")
                .and_then(|provider| provider.host_id.clone())
                .as_deref(),
        ...
        assert_eq!(
            migrated
                .providers
                .iter()
                .find(|provider| provider.provider_id == "ollama_local")
                .and_then(|provider| provider.auth_binding_id.clone())
                .as_deref(),
        ...
        with_cleared_provider_env(|| {
            ...
            assert_eq!(
                policy
                    .batch_window
                    .as_ref()
                    .and_then(|window| window.interval_seconds),
                Some(60)
            );
        });
    }
    ...
    #[test]
    fn resolve_runtime_state_skips_non_executable_bound_provider() {
        with_cleared_provider_env(|| {
            let state_path = std::env::temp_dir().join(format!(
                "provider-runtime-boundary-{}.json",
                std::process::id()
            ));
            ...
            std::env::set_var(DEFAULT_PROVIDER_RUNTIME_URL_ENV, "http://127.0.0.1:11434");
            std::env::set_var("NOSTRA_AGENT_MODEL", "llama3.1:8b");

            save_provider_registry_state(&ProviderRegistryState {
                providers: vec![
                    ProviderRuntimeSettings {
                        ...
                        profile_id: Some("llama3.1:8b".to_string()),
                        instance_id: Some(
                            "ollama_vps_primary__ssh_root_204.168.175.150".to_string(),
                        ),
                        device_id: None,
                ...
                runtime_hosts: vec![
                    ...
                    RuntimeHostRecord {
                        ...
                        host_kind: RuntimeHostKind::Local,
                        endpoint: "http://127.0.0.1:11434".to_string(),
                        locality_kind: ProviderLocalityKind::Local,
                        device_id: Some("macbook-pro".to_string()),
                        environment_id: Some("local-dev".to_string()),
                        health: None,
                ...
                auth_bindings: vec![
                    ...
                    AuthBindingRecord {
                        auth_binding_id: "auth.none.ollama_local".to_string(),
                        target_kind: AuthBindingTargetKind::Provider,
                        target_id: "ollama_local".to_string(),
                        auth_type: AuthBindingType::None,
                        label: Some("Ollama Local".to_string()),
                        source: Some("manual".to_string()),
                        secret: String::new(),
                        has_secret: false,
                        created_at: Some("2026-03-25T00:00:00Z".to_string()),
                ...
                execution_bindings: vec![ExecutionBindingRecord {
                    binding_id: DEFAULT_LLM_BINDING_ID.to_string(),
                    provider_type: "Llm".to_string(),
            ...
            let resolved = resolve_provider_runtime_state();
            assert_eq!(resolved.provider_id, "ollama_local");
