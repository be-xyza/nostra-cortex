...
impl AgentService {
    ...
    async fn run_legacy_chat(request: &RuntimeChatRequest) -> Result<RuntimeChatResponse, String> {
        ...
        let decoded =
            crate::services::ic_client::IcClient::unwrap_candid_string(&response)
                .unwrap_or(response);
        let agent = ChatAgentIdentity {
...
mod tests {
    use super::{
        ...
        chunk_chat_response, ensure_thread_context, ChatContentPart, ChatContextBlock,
        ChatHistoryTurn, ChatSourceAnchor, RuntimeChatRequest,
    };
    use serde_json::json;
    ...
    fn ensure_thread_context_preserves_existing_thread_header() {
        let message = "Conversation thread: thread-42\nUser request: Summarize the current status.".to_string();
    ...
    fn build_chat_message_payload_embeds_thread_id() {
        ...
        assert!(payload.contains(r#""text":"Conversation thread: thread-42\nUser request: Summarize the current status."#));
    }
    ...
    fn chunk_chat_response_breaks_long_messages_on_word_boundaries() {
        ...
        assert_eq!(chunks.join(" "), message.split_whitespace().collect::<Vec<_>>().join(" "));
    }
...
fn build_runtime_chat_instructions(request: &RuntimeChatRequest) -> String {
    let mut sections = vec![
        ...
        "Answer the user using the supplied thread history and canonical heap context bundle.".to_string(),
        "Ground claims in the provided context blocks and source anchor when available.".to_string(),
