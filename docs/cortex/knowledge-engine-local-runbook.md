...
- Agent identity override: `NOSTRA_AGENT_IDENTITY_PEM` (defaults to ICP CLI identity discovery)
...
cd ..
TERM=xterm-256color COLORTERM=truecolor icp build
```

### Cortex Gateway Reliability Check

Run the launcher/gateway strict reliability check:

```bash
bash /Users/xaoj/ICP/scripts/cortex-daemon-closeout-check.sh
```

What it verifies:
1. `cargo test --manifest-path /Users/xaoj/ICP/cortex/apps/cortex-eudaemon/Cargo.toml --tests`
2. Single-instance launcher lock behavior (`already running` on second launch)
3. Readiness endpoint returns `ready=true`:
   - `http://127.0.0.1:3000/api/system/ready` (or `CORTEX_GATEWAY_PORT`)
4. No panic signatures in newly appended `cortex_runtime.log` lines for the run
5. Run artifact is written to `logs/testing/runs/<run_id>.json`
6. Compatibility section is refreshed at:
   - `logs/testing/test_gate_summary_latest.json`
   - key: `.compatibility.cortex_desktop_closeout`
...
1. It prints:
   ...
   - `icp network status` from `icp-cli` (the canonical CLI, typically exposed as the `icp` command)
2. If a healthy local replica is already running and `--reuse-replica=true`, it reuses that replica.
3. If port `4943` is occupied but `icp-cli` / `icp network status` fails to recognize the repo-managed network, the script exits with manual remediation steps instead of killing processes. A healthy unmanaged local replica may still be present.
