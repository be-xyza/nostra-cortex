...
## Managed `icp-cli` Service Promotion On March 20, 2026

The earlier scratch probe at `/srv/nostra/eudaemon-alpha/tmp/icp-replica-probe` was intentionally replaced with a repo-owned and `systemd`-managed IC lane.

### Repo-Owned Assets Synced To The VPS

```text
/srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha/icp.yaml
/srv/nostra/eudaemon-alpha/repo/ops/hetzner/systemd/cortex-icp-network.service
/srv/nostra/eudaemon-alpha/repo/ops/hetzner/systemd/cortex-gateway.service
/srv/nostra/eudaemon-alpha/repo/scripts/run_cortex_icp_network.sh
/srv/nostra/eudaemon-alpha/repo/scripts/stop_cortex_icp_network.sh
```

Canonical repo-owned network manifest:
...
### Live Cutover Timeline

Initial cutover attempt started at `2026-03-20T20:09:34Z`.

Commands used:

```bash
cat >/etc/systemd/system/cortex-icp-network.service <<'EOF'
[Unit]
Description=Managed local ICP network for Eudaemon Alpha Phase 6
After=network-online.target
Wants=network-online.target
Before=cortex-gateway.service

[Service]
Type=oneshot
User=nostra
Group=nostra
WorkingDirectory=/srv/nostra/eudaemon-alpha/repo
EnvironmentFile=/srv/nostra/eudaemon-alpha/config/eudaemon-alpha.env
ExecStart=/srv/nostra/eudaemon-alpha/repo/scripts/run_cortex_icp_network.sh
ExecStop=/srv/nostra/eudaemon-alpha/repo/scripts/stop_cortex_icp_network.sh
RemainAfterExit=yes
StandardOutput=append:/srv/nostra/eudaemon-alpha/logs/cortex-icp-network.log
StandardError=append:/srv/nostra/eudaemon-alpha/logs/cortex-icp-network.log

[Install]
WantedBy=multi-user.target
EOF

cd /srv/nostra/eudaemon-alpha/tmp/icp-replica-probe && icp network stop
systemctl enable --now cortex-icp-network.service
```

Observed first failure at `2026-03-20T20:09:57Z`:

```text
Error: failed to create network directory
Caused by:
    0: Filesystem operation failed at /srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha/.icp/cache/networks/local
...
```bash
chown -R nostra:nostra /srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha
install -d -o nostra -g nostra /srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha/.icp/cache/networks
systemctl reset-failed cortex-icp-network.service
systemctl start cortex-icp-network.service
systemctl restart cortex-gateway.service
```

Successful managed-lane activation completed at `2026-03-20T20:10:42Z`.

Observed service state:

```text
cortex-icp-network.service: active (exited)
ExecStart=/srv/nostra/eudaemon-alpha/repo/scripts/run_cortex_icp_network.sh
launcher=/home/nostra/.local/share/icp-cli/pkg/network-launcher/.../icp-cli-network-launcher
state_dir=/srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha/.icp/cache/networks/local/state
bind=127.0.0.1:4943
```

Observed managed network status under `nostra`:

```json
{
  "managed": true,
  "api_url": "http://localhost:4943/",
  "gateway_url": "http://localhost:4943/",
...
Readiness at `2026-03-20T20:11:03Z`:

```json
{"ready":true,"gateway_port":3000,"icp_network_healthy":true,"dfx_port_healthy":true,"notes":[]}
```

Status at `2026-03-20T20:11:03Z`:

```json
{"icp_cli_running":true,"dfx_running":true,"version":"icp 0.2.1","replica_port":4943}
```

Smoke at `2026-03-20T20:11:03Z`:
...
Verified operator auth after cutover at `2026-03-20T20:11:03Z`:
...
Verified steward export still correctly blocked after cutover at `2026-03-20T20:11:03Z`:
...
### Passive Browser Re-Validation After The Frontend Guard

A second browser/operator validation was run against the live gateway with the updated local `cortex-web` frontend so contribution focus could be verified without auto-starting new runs.

Bridge commands used:

```bash
ssh -N -L 3300:127.0.0.1:3000 root@204.168.175.150
python3 <local header-injecting proxy on 127.0.0.1:3301 adding x-ic-principal: 2vxsx-fae>
VITE_CORTEX_GATEWAY_URL=http://127.0.0.1:3301 VITE_SPACE_ID=01KM4C04QY37V9RV9H2HH9J1NM npm -C /Users/xaoj/ICP/cortex/apps/cortex-web run dev -- --host 127.0.0.1 --port 4274
```

Observed browser result at `2026-03-20T20:11:55Z`:

```json
{
  "url": "http://127.0.0.1:4274/contributions?contribution_id=proposal-alpha",
  "launchPosts": 0,
  "errorVisible": true
}
```
...
- contribution focus stayed passive on the live gateway
- `Launch Live Run` remained visible until an explicit operator action
- no `POST /api/kg/spaces/.../agents/contributions` calls were emitted implicitly
- clicking `Export Steward Packet` surfaced the live `DPUB_PACKET_STEWARD_REQUIRED` error in the UI

Updated browser screenshot:

- [/Users/xaoj/ICP/logs/cortex/contributions-cockpit-vps-operator-managed-icp-2026-03-20T20-11-55Z.png](/Users/xaoj/ICP/logs/cortex/contributions-cockpit-vps-operator-managed-icp-2026-03-20T20-11-55Z.png)

### Status Change Relative To The Earlier Note

- The IC tool lane is now repo-owned and service-managed. The earlier scratch probe risk is closed.
- The previously observed contribution-focus fan-out no longer reproduced in the passive browser validation once the explicit launch guard was in place.
- The 25 historical `proposal-alpha` runs created during the earlier operator validation remain on the host as failed records; they were not deleted.
...
- The 25 historical `proposal-alpha` runs created during the earlier operator validation remain present as failed records and should be reviewed or explicitly marked as validation noise.
- The passive launch guard is validated from the updated local `cortex-web` frontend against the live gateway; any other operator-distributed frontend bundle still needs the same code revision before it can claim this behavior.
...
2. Review the 25 historical failed `proposal-alpha` runs and decide whether to archive or retain them as validation evidence.
3. Execute at least one controlled contribution-graph pipeline run on-host so graph-run history becomes non-empty and no longer relies only on the seeded baseline artifact.
4. If a packaged operator frontend exists outside this local `cortex-web` workflow, roll the passive launch guard into that bundle before claiming the fan-out issue is closed everywhere.
