...
## Managed Local IC Lane

The canonical Hetzner IC lane is now a repo-owned `icp-cli` managed network rather than an ad hoc scratch probe. Required assets:

- [`icp.yaml`](/Users/xaoj/ICP/ops/hetzner/icp/eudaemon-alpha/icp.yaml)
- [`cortex-icp-network.service`](/Users/xaoj/ICP/ops/hetzner/systemd/cortex-icp-network.service)
- [`run_cortex_icp_network.sh`](/Users/xaoj/ICP/scripts/run_cortex_icp_network.sh)
- [`stop_cortex_icp_network.sh`](/Users/xaoj/ICP/scripts/stop_cortex_icp_network.sh)

Live service shape:

- `cortex-icp-network.service` runs as the same service user as `cortex-gateway.service`
- network state lives under `/srv/nostra/eudaemon-alpha/repo/ops/hetzner/icp/eudaemon-alpha/.icp/`
- gateway dependency ordering is explicit: `cortex-gateway.service` `Wants=` and `After=` `cortex-icp-network.service`
- port `4943` must remain loopback-only

If `4943` is occupied but `icp network status` does not recognize the configured managed network, stop and remediate manually rather than killing arbitrary processes.
...
3. Install the root repo service assets on the server:
...
2. If the IC lane assets changed, sync [`icp.yaml`](/Users/xaoj/ICP/ops/hetzner/icp/eudaemon-alpha/icp.yaml) plus the `cortex-icp-network.service` scripts and restart `cortex-icp-network.service` first.
3. Restart `cortex-gateway.service` after the repo update lands.
4. Run the read-only smoke verifier before any steward mutation checks:
...
5. Treat these smoke outcomes differently:
...
6. After smoke passes, validate operator behavior through a browser session:
