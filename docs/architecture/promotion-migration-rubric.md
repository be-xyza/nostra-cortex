...
`cortex-web` started as a `rename with redirect` migration but was completed as
`rename now` once the remaining alias surface was small enough to remove:
...
- removing the final `/gallery` alias stops the wrong canonical term from
  remaining discoverable after the migration is already understood
