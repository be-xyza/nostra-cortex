...
|---|---|---|
| `gallery` | Strong user expectation of browsing curated artifacts or media. Reusing it for the layout comparison surface would cause future collision. | Reserve for a future user-facing browsing or collection concept. Keep `/gallery` only as a compatibility redirect during migration. |
...
`/labs/layout-catalogue` in `cortex-web`. The legacy `/gallery` route should be
treated only as a compatibility alias during migration, not as the intended
surface name.
