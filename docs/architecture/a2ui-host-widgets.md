...
- `ChatMessageSurface`
  - Hosts inline mixed-content message rendering inside conversation bubbles using the shared React A2UI interpreter and pointer/text parts.
- `ContributionGraph`
...
- Transitional exception:
  - `/conversations` may remain a React-owned projection route while it is backed entirely by canonical server conversation projections and does not become a second authority for thread history.
