...
### Phase 1: Ingestion Pipeline (Delegated to 127)
- [x] **Define Schema**: Implemented via `127-cortex-native-repo-ingestion` `ingestion_registry.toml`.
- [x] **Implement Ingestion Workflow**: Handled natively by Cortex Memory FS temporal sandboxing.
- [x] **Data Storage**: Sandboxes managed ephemerally; metadata stored in Knowledge Graph.

### Phase 2: Analysis Engine (Deep Dive via 126 & 013)
- [ ] **Agent Task Integration**: Define `agent-task-ast-parse` and `agent-task-llm-summarize` to run securely within the Cortex Agent Harness (`126`).
- [ ] **Knowledge Graph Construction**:
    -   [ ] Map outputs to `008` standardized nodes (`Library`, `Idea`, `Proposal`).
    -   [ ] Implement initial Force-Directed Graph using D3.js or Sigma.js.
