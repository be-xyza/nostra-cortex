...
- [x] Record gateway behavior baseline
  ...
  - Store fixtures in `nostra/apps/cortex-desktop/tests/fixtures/gateway_baseline/`
...
- `nostra/libraries/cortex-domain/` crate — pure, sync, `#![forbid(unsafe_code)]`, compiles to wasm32
- `nostra/libraries/cortex-runtime/` crate — async orchestration, restricted tokio, compiles to wasm32
- `nostra/libraries/cortex-ic-adapter/` crate — ICP adapter
...
- `cortex_ux.rs` (1,020 lines) → **split**: pure scoring to `cortex-domain/ux/`, orchestration to `cortex-runtime/ux/`, persistence to host
...
> [!NOTE]
> **Matrix freshness (2026-02-14)**: Several extraction targets have grown since initial
> classification. `cortex_ux.rs` (32KB), `streaming_transport.rs` (41KB), and
> `acp_protocol.rs` (34KB) are significantly larger than when first assessed. Line counts
> in per-file analysis docs may be stale. Re-validate line-level split decisions before
> starting each phase.
...
| `cortex_ux.rs` | 1,020 | 4 | **S** | 3 env, 2 utc, 1 fs |
| `acp_protocol.rs` | 1,015 | 2 | **S** | 6 env, 4 once |
