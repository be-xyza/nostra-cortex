...
title: "Initiative 118 \u2014 Architectural Decisions"
type: general
project: nostra
status: draft
authors:
...
1. `cargo check --manifest-path cortex/Cargo.toml -p cortex-domain` (PASS)
2. `cargo check --manifest-path cortex/Cargo.toml -p cortex-runtime` (PASS)
3. `cargo check --manifest-path cortex/Cargo.toml -p cortex-desktop --bin gateway_server` (PASS)
4. `cargo check --manifest-path cortex/Cargo.toml -p cortex-desktop --bin cortex_desktop` (PASS)
...
3. `cargo test --manifest-path cortex/Cargo.toml -p cortex-runtime` (PASS, gateway dispatcher tests included)

---

## ADR-024: Phase 5 Gateway Extraction Closure
...
Close Initiative 118 Phase 5 as implementation-complete in workspace with:
1. Runtime template-based route matching and contract-derived route metadata.
2. Full `/api/*` runtime-dispatch middleware path in gateway server.
3. Legacy host behavior preserved behind explicit runtime-host passthrough.
4. Phase-5 target singleton callsites removed from gateway/resilience/agent paths.

### Implemented scope

1. Runtime gateway foundations:
   - Added `path_template` and `path_params` to `GatewayRequestEnvelope`.
   - Added `GatewayRouteMetadata`.
   - Added `cortex-runtime::gateway::routes` matcher with static-over-param precedence and ambiguity rejection.
   - Added additive `GatewayHostAdapter::resolve_route(...)` default contract.
2. Desktop runtime host contract registry:
   - Loads `GATEWAY_PROTOCOL_CONTRACT_2026-02-16.json`.
   - Resolves route metadata from inventory-locked contract descriptors.
3. Gateway server runtime dispatch:
   - Added `/api` middleware interception with dispatcher execution.
   - Added explicit bypass header path (`x-cortex-runtime-legacy-bypass`) for host passthrough loop.
   - Legacy `/api/health` handler is direct to prevent recursive self-dispatch.
4. Target singleton callsite cleanup:
   - `gateway/server.rs` local-gateway flows now use runtime-host wrappers.
   - `services/resilience_service.rs` now uses runtime-host probe wrapper.
   - `services/agent_service.rs` now uses runtime-host online/mutation wrappers.
5. Phase 5 route worklist artifact added:
   - `research/118-cortex-runtime-extraction/PHASE_5_ROUTE_WORKLIST_2026-02-16.md`

### Validation evidence

1. `research/118-cortex-runtime-extraction/PHASE_5_COMPLETION_EVIDENCE_2026-02-16.md`
2. `bash scripts/check_gateway_parity_inventory_sync.sh` (PASS: `123/123`, exemptions `0`)
3. `bash scripts/check_gateway_protocol_contract_coverage.sh` (PASS: `123/123`)
4. `bash scripts/run_cortex_runtime_freeze_gates.sh` (PASS)
5. `bash scripts/check_118_pr_evidence.sh --pr-body-file tests/fixtures/pr_evidence/valid.md` (PASS)
6. `bash tests/scripts/test_check_118_pr_evidence.sh` (PASS)

### Rationale

Phase 5 closure requires runtime ownership of gateway protocol semantics without breaking endpoint parity or governance controls. This decision formalizes full runtime path activation while preserving host compatibility and existing freeze/evidence discipline.

---

## ADR-025: Post-118 Runtime Hardening, Adapterization, and In-Process Dispatch Default
...
Implement post-118 hardening slices immediately after Phase-5 closure to stabilize runtime dispatch and remove remaining local-gateway orchestration coupling.

### Implemented scope

1. Gateway response contract enriched in `cortex-runtime`:
   - `route_template` field added to `GatewayResponseEnvelope`.
   - `dispatch_error` classification surface added (`GatewayDispatchError`, `GatewayDispatchErrorClass`).
2. Runtime dispatch behavior hardened:
   - Route-template tagging is now stamped at dispatcher layer.
   - Upstream 4xx/5xx fallback classification is normalized.
3. Desktop runtime host hardening:
   - Legacy passthrough now enforces explicit request/response header filtering.
   - Failure taxonomy added for timeout/network/non-json upstream failures.
4. In-process legacy dispatch path introduced and set as runtime default:
   - Dispatch mode is read from `CORTEX_GATEWAY_LEGACY_DISPATCH_MODE`.
   - Default mode: `in_process`; fallback mode: `http_loopback`.
5. Local-gateway adapterization:
   - Added runtime port: `LocalGatewayOrchestrationAdapter`.
   - Added runtime local orchestration module: `cortex-runtime/src/gateway/local.rs`.
   - Added desktop singleton bridge: `services/local_gateway_bridge.rs`.
   - `runtime_host.rs` no longer calls `local_gateway::get_gateway()` directly.
6. New guardrails and freeze-gate integration:
   - `scripts/check_gateway_contract_descriptors_strict.sh`
   - `scripts/check_local_gateway_singleton_boundary.sh`
   - Both wired into `scripts/run_cortex_runtime_freeze_gates.sh`.
...
3. `cargo check --offline --manifest-path /Users/xaoj/ICP/cortex/Cargo.toml -p cortex-desktop --bin gateway_server` (PASS)
4. `CARGO_NET_OFFLINE=true bash /Users/xaoj/ICP/scripts/run_cortex_runtime_freeze_gates.sh` (PASS)
5. `bash /Users/xaoj/ICP/scripts/check_118_pr_evidence.sh --pr-body-file /Users/xaoj/ICP/tests/fixtures/pr_evidence/valid.md` (PASS)

### Rationale

Phase-5 completion established runtime extraction semantics but left hardening and adapter ownership gaps. This ADR closes those gaps without changing endpoint inventory or parity expectations.

---

## ADR-026: GSMS Prerequisite and GSMS-0 Foundation Activation
...
Activate GSMS prerequisite implementation in `cortex-domain` and publish GSMS-0 deterministic harness artifacts so Layer-3 execution can proceed under explicit gate discipline.

### Implemented scope

1. Layer-0 graph primitives expanded:
   - `Graph`, `Node`, richer `EdgeKind`, deterministic root hash.
   - Extended traversal and structural diff capabilities for simulation use.
2. Layer-1 SIQS implementation added:
   - `integrity/rule.rs`, `integrity/predicate.rs`, `integrity/engine.rs`.
   - Deterministic `evaluate_all` and violation surface.
3. 119 dependency surface added:
   - `integrity/commons.rs` with `CommonsEnforcementMode` (`shadow` and `warn_or_block`) and deterministic outcome model.
4. GSMS-0 deterministic harness added:
   - `simulation/scenario.rs` (YAML parser + canonical sequence)
   - `simulation/session.rs` (deterministic replay + cap abort + bench payload)
5. Canonical scenario suites added:
   - `research/118-cortex-runtime-extraction/scenarios/gsms0/constitutional_unit.yaml`
   - `research/118-cortex-runtime-extraction/scenarios/gsms0/commons_stress.yaml`
   - `research/118-cortex-runtime-extraction/scenarios/gsms0/governance_replay.yaml`
6. Portfolio artifacts added:
   - `GSMS_0_EXECUTION_PLAN_2026-02-16.md`
   - `GSMS_BENCH_MAPPING_CONTRACT_2026-02-16.md`
   - `GSMS_PREREQ_GATE_2026-02-16.md`

### Validation evidence

1. `cargo test --offline --manifest-path /Users/xaoj/ICP/cortex/Cargo.toml -p cortex-domain` (PASS, includes integrity/simulation tests)
2. Determinism test: `simulation::session::tests::deterministic_session_replay_is_stable_for_same_seed` (PASS)
3. Scenario parser test: `simulation::scenario::tests::parse_and_canonicalize_yaml_scenario` (PASS)

### Rationale

GSMS remained policy-complete but implementation-gated. This ADR establishes concrete deterministic foundations and dependency contracts while preserving no-chronicle and no-wall-clock constraints for GSMS-0.
>>>>>>> Stashed changes
