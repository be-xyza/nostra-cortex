...
`Utc::now()` / `SystemTime::now()` / `Instant::now()` found in **15 runtime-portable services**, **43+ call sites**:
...
| `cortex_ux.rs` | 3 | Evaluation IDs, ISO timestamps |
| `acp_event_sink.rs` | 1 | Event timestamps |
| `acp_event_projector.rs` | 2 | Projection timestamps |
...
| `artifact_collab_crdt.rs` | 3 | CRDT state timestamps |
...
├── cortex_ux (pure evaluation) → runtime/ux/
