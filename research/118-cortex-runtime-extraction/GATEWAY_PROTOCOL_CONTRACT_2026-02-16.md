...
- Baseline refresh (180 endpoints) includes system operator endpoints:
  - `GET /api/system/agents/runs` (agent run inventory; operator-only)
  - `GET /api/system/agents/runs/:space_id/:run_id` (single run readback; operator-only)
  - `GET /api/system/llm-adapter/status` (best-effort health; additive fields only)
  - `GET /api/system/logs/streams` (curated system log streams inventory; operator-only)
  - `GET /api/system/logs/streams/:stream_id/tail` (cursor-based tail; role-gated per stream)
