...
- [x] M23 is complete.
- [x] M24 is complete.
- [x] Ingestion quality gates from `037`, `042`, and `051` are green enough for controlled evaluation.

### Checklist

- [x] Define the retrieval pipeline stages: embed, find, extract, reason.
- [x] Define which stages consume graph triples versus embeddings versus provenance.
- [x] Reuse hybrid scoring concepts from `042` instead of inventing a separate ranking stack.
- [ ] Add retrieval-policy inputs from `M23` to the pipeline.
- [x] Add citation output so answers can show source lineage.
- [x] Start with a curated corpus and known-answer evaluation set.
- [x] Benchmark against pure vector retrieval.
- [x] Benchmark against the current local knowledge-engine path from `037`.
- [ ] Add failure modes for low-confidence or incomplete graph coverage.
- [x] Keep the first version bounded to controlled corpora and operator review.
...
- [x] Compare the pilot against the currently captured `037` ask/search path in a shared evaluation artifact.
...
### Initiative 078 Closeout Status

- [x] Expand the fixed benchmark suite to cover relation-traversal, scope-constrained, provenance-sensitive, and evidence-support cases.
- [x] Emit a case-aligned shared evaluation artifact against the current `037` knowledge path.
- [x] Validate the `136` Explore handoff against both research-space and agent-scope topology fixtures.
- [x] Record graph-promotion criteria and explicitly defer new graph-native evidence classes until later evidence justifies them.

### Exit Criteria

- [x] Graph RAG improves answer quality on multi-hop questions.
- [x] Retrieval remains explainable and lineage-aware.
- [x] The pipeline is still compatible with Nostra-native contracts.
