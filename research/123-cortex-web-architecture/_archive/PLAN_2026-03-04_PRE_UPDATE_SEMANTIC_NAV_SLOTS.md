...
7. Initiative 130 alignment: space-level capability overlays compile deterministic per-space navigation/surfacing plans from the platform catalog + steward-governed space graph.
...
### Phase 6: Space Capability Overlay and Navigation Plan Compilation (Initiative 130)
- Introduce `PlatformCapabilityCatalog` + `SpaceCapabilityGraph` overlay architecture.
- Expose additive APIs for per-space capability graph retrieval/update and navigation plan compilation.
- Enforce steward-gated structural overlay edits with lineage metadata.
- Keep `/api/system/capability-graph` backward-compatible while web shell can consume compiled per-space navigation plans.
