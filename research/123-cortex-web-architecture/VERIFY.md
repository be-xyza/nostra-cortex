...
- [x] `bash scripts/check_cortex_dual_host_parity.sh` passes when gateway is running on `127.0.0.1:3000`.
...
## Capability Navigation Graph Stabilization Evidence (Latest Run: 2026-03-01)
- [x] `/api/spaces/create` is bypassed from runtime dispatch and returns success from live gateway.
- [x] `/api/system/capability-graph` returns schema’d projection with `schema_version`, `generated_at`, `source_of_truth`, `nodes`, and `edges`.
- [x] `/api/system/ux/workbench` accepts contextual query hints (`intent`, `density`, `node_id`) and surfaces contextual inspector text.
- [x] `gateway_server` binary starts real gateway service instead of pending/no-op behavior.

## Notes
- Some local checks requiring loopback network access were executed with elevated sandbox permissions in agent runtime.
