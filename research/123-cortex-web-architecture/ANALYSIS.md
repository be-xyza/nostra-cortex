...
> **Update 2026-03-01**: The Dioxus UI layer has now been explicitly deprecated per `DEC-123-004`. The `cortex-desktop` executable operates as a headless daemon/gateway, while `cortex-web` (React/A2UI) is the unified frontend strategy.
...
- **Tech Stack:** Headless Rust Daemon (Previously Dioxus - *Deprecated via DEC-123-004*)
- **Purpose:** Full local runtime, Labs mode, Simulation + Godot integration, Power users.
- **Value:** Retains the deep vision of a pure, single-language, memory-safe, sovereign operating node executing Temporal workflows, delegating rendering entirely to `cortex-web`.
