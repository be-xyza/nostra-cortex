...
title: 'Requirements: Provider Economics, Routing, and Metering'
type: general
project: nostra
status: draft
authors:
...
# Requirements: Provider Economics, Routing, and Metering
...
| FR-2.6 | Budget posture inputs for routing (`Lean`, `Balanced`, `Quality-first`) | P1 |
...
| FR-5.6 | Usage and expense summaries for provider families and spaces | P1 |
...
### FR-7: DoubleWord Batch Strategy
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-7.1 | Explicit batch policy contract for DoubleWord providers | P0 |
| FR-7.2 | Support interval, time-window, and scope-bounded batching | P0 |
| FR-7.3 | Include ordering and dedupe keys for idempotent replay | P0 |
| FR-7.4 | Batch policy should compose with provider family/profile metadata | P1 |
...
type BatchWindow = record {
    interval_seconds: opt nat64;
    max_items: opt nat64;
    max_age_seconds: opt nat64;
    timezone: opt text;
};

type ProviderBatchPolicy = record {
    provider_family_id: text;
    provider_profile_id: opt text;
    cadence_kind: BatchCadenceKind;
    scope_kind: BatchScopeKind;
    flush_policy: BatchFlushPolicy;
    ordering_key: opt text;
    dedupe_key: opt text;
    batch_window: opt BatchWindow;
};
```
