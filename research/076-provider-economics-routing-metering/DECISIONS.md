...
title: 'Decisions: Provider Economics, Routing, and Metering'
type: general
project: nostra
status: draft
authors:
...
# Decisions: Provider Economics, Routing, and Metering

## Decision Log

### D-000: Initiative Rename and Scope
**Date**: 2026-03-23
**Status**: DECIDED
**Decision**: Reframe this initiative as provider economics, routing, and metering rather than a single OpenRouter feasibility study.

**Context**
The scope now includes provider onboarding, model catalogs, pricing, budget posture, and DoubleWord batch strategy.

**Decision**
Use the new umbrella title for 076. Keep OpenRouter as one provider family and keep Cobudget as the source platform for reusable financial patterns.

**Consequences**
- Research naming aligns with actual scope.
- Financial analysis can reference Cobudget without inventing a new source platform name.
- DoubleWord can share the same routing and metering umbrella.
...
### D-014: Budget Posture Is a Routing Input
**Date**: 2026-03-23
**Status**: DECIDED
**Decision**: Treat budget posture as an explicit routing input alongside provider capability and cost.

**Context**
The same provider catalog must support different operating modes such as cost-first, balanced, and quality-first usage.

**Decision**
Use simple posture labels in the product and keep the derivation logic separate from provider discovery.

**Consequences**
- Routing can recommend cheaper or higher-quality defaults without changing the catalog.
- Cost estimates become contextual instead of one-size-fits-all.
- Usage reporting can explain why a provider was selected.

### D-015: DoubleWord Requires a First-Class Batch Policy
**Date**: 2026-03-23
**Status**: DECIDED
**Decision**: Model DoubleWord batching with an explicit batch policy contract rather than implicit config strings.

**Context**
DoubleWord supports multiple batching strategies based on time primitives and batch scope.

**Decision**
Add a provider batch policy that includes cadence, scope, flush policy, ordering key, dedupe key, and provider family/profile identifiers.

**Consequences**
- Interval, time-window, and scope-bounded strategies can be expressed consistently.
- Batch behavior stays additive and compatible with existing provider records.
- Strategy selection can evolve without breaking the provider registry shape.
