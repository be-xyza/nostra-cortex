...
## DEC-138-010: Runtime types enforce protected-resource non-emission invariants

**Date**: 2026-05-02

**Decision**: Cortex runtime code may mirror Phase 4 protected-resource contracts as serializable domain types only if the types preserve the non-emission invariant: raw value export remains forbidden, grants require audit, and visible records use handles, fingerprints, redacted previews, status, or sealed artifact references.

**Rationale**: Contract documents are useful but insufficient once implementation begins. Runtime types should make the safe shape easy to use and the unsafe shape hard to accidentally serialize.

**Implications**:
- Cortex may serialize protected-resource metadata, `SecretRef` handles, grants, sealed invocation requests, and use events.
- Runtime validation rejects raw export policy changes and non-audited grants.
- This does not yet authorize runtime vault access or agent-visible raw values.
