# Research Initiative 124: AGUI Heap Mode Methodology

## 1. Executive Summary

This research investigates the adoption of the "Heaper methodology" (tag-centric, bottom-up block organization via CRDTs) as a primary "mode" for our AGUI (Agent-Generated UI) and A2UI streaming interfaces, specifically tailored for Personal Knowledge Management (PKM). 

The goal is to transition A2UI from an ephemeral, chat-centric paradigm to a persistent, organically developing ecosystem of components. By translating Heaper's "Block" and "Heap" concepts into Nostra/Cortex architectures, agent-generated UIs become durable, interconnected artifacts within the user's workspace.
...
## 2. Methodology Analysis: Heaper vs. AGUI
...
### 2.2 The AGUI / A2UI Challenge
Currently, AGUI streams dynamic UI components (charts, forms, interactive widgets) into a linear chat feed. 
...
- **User Curation:** The user can interact with the AGUI component, annotate it with rich text (just like Heaper blocks), and retag it. The A2UI component becomes a living artifact in the user's PKM graph.
...
| Heaper Concept | Nostra Primitive | A2UI/AGUI Integration |
|----------------|------------------|-----------------------|
...
How to accomplish "Heap Mode" for AGUI:
...
1. **A2UI Contribution Schema:** Define a new `Contribution` payload type specifically for `AGUI_State`. This schema stores the initial A2UI rendering JSON and a serialized CRDT blob for ongoing state changes.
...
2. **Heap ViewSpec:** Create a `ViewSpec` in Cortex that renders a Space not as a document tree, but as a reverse-chronological, filterable masonry grid of AGUI Blocks (The Heap).
