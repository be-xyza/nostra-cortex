...
- [ ] `cargo check --manifest-path /Users/xaoj/ICP/nostra/Cargo.toml -p frontend` *(blocked: `index.crates.io` DNS/network resolution in current environment)*
- [ ] `cargo check --offline --manifest-path /Users/xaoj/ICP/nostra/Cargo.toml -p frontend` *(now reaches frontend compile stage; blocked by existing workspace dependency/module parity issues)*
