...
- [ ] Resolve workspace dependency chain requiring yanked `keccak` via `nostra-test-kit -> ic-agent 0.45` path in CI-connected environment.
...
- ❌ `cargo check --offline --manifest-path /Users/xaoj/ICP/nostra/Cargo.toml -p frontend` (yanked dependency chain via `nostra-test-kit` -> `ic-agent 0.45`)
