...
| Phase 3: Upgrade Protocol | 🟡 In Progress (gated) | Upgrade notice/history/event scaffolding shipped; full GlobalEvent + proposal workflow integration remains. |
| Phase 4: Automation Hooks | 🟡 In Progress (gated by 013) | Policy + manual run surfaces shipped behind explicit enable flag; workflow-governance bridge pending. |
| Phase 5: Canonical Commons v1 | ⏳ Pending | Requires curated v1 ruleset package + onboarding flow. |
| Phase 6: Simulation Validation Gate | 🟡 In Progress (gated) | Simulation policy/report/ratification gate surfaces shipped; GSMS-integrated ratification flow pending. |
