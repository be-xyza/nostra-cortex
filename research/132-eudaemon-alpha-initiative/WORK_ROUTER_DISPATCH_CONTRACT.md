...
- [AgentHarnessRegistryV1.schema.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/schemas/AgentHarnessRegistryV1.schema.json)
- [AgentProfileV1.schema.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/schemas/AgentProfileV1.schema.json)
- [AgentPromotionGateV1.schema.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/schemas/AgentPromotionGateV1.schema.json)
- [AgentRunEvidenceV1.schema.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/schemas/AgentRunEvidenceV1.schema.json)
- [AgentActivationRoadmapV1.schema.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/schemas/AgentActivationRoadmapV1.schema.json)
- [agent_runtime_boundaries.v1.json](/Users/xaoj/ICP/research/132-eudaemon-alpha-initiative/examples/agent_runtime_boundaries.v1.json)

Local validation command:

```bash
bash scripts/check_work_router_dispatch.sh
bash scripts/check_agent_operating_model.sh
```
...
| `AgentHarnessRegistryV1` | Recommendation-only comparison record for external or native agent harnesses such as Hermes Agent, OpenClaw, ZeroClaw, Monopi, Codex, or future Cortex-native hosts |
| `AgentProfileV1` | Project-facing agent/profile record with harness binding, authority ceiling, forbidden actions, transport bindings, health, and lifecycle state |
| `AgentPromotionGateV1` | Evidence gate for promoting, restricting, demoting, or retiring an agent/profile |
| `AgentRunEvidenceV1` | Per-run profile evidence proving what an agent actually did under its declared ceiling |
| `AgentActivationRoadmapV1` | Conditional future-agent roadmap so new agents activate only when platform readiness and evidence gates exist |
| `agent_runtime_boundaries.v1.json` | Live runtime/profile boundary checks that compare declared agent authority to actual tool access |
...
Agent harnesses are replaceable substrates:

- Hermes Agent, OpenClaw, ZeroClaw, Monopi, Codex, and future Cortex-native hosts may be tracked and compared through `AgentHarnessRegistryV1`.
- Hermes Gateway, Telegram direct, Cortex Web, CLI, Matrix, email, and webhooks are peer dispatch adapters. A mature gateway can be practical without becoming authority.
- Project-facing agents must instantiate `AgentProfileV1` before promotion and must pass `AgentPromotionGateV1` evidence before their lifecycle state is raised.
- Active or pilot profiles must declare a `runtimeBoundary`. Locally inspectable profiles and agent-like systemd services must be covered by `agent_runtime_boundaries.v1.json`; project-looking Hermes profiles or agent-like Cortex services discovered outside that boundary fail validation.
...
### Slice 2: Transport Adapter and Agent Operating Model

- Keep Telegram direct, Hermes Gateway, Cortex Web, CLI, Matrix, email, and webhook paths behind `DispatchTransport`.
...
- Validate `AgentHarnessRegistryV1`, `AgentProfileV1`, and `AgentPromotionGateV1` so harness choice remains evidence-driven.
...
Hermes Gateway follows the same adapter contract. A Hermes profile message must carry or resolve to a specific pending request/envelope before it can become a `DispatchDecisionV1`. Known aliases such as `approved` may normalize to `approve`; typos or ambiguous text such as `apprve please` are recorded for unknown-response review. Hermes Gateway does not bypass WorkRouter, does not own authority, and does not mutate source or runtime state.
