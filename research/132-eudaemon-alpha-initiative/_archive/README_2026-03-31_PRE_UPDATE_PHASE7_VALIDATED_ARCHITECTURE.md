...
| 3 | Native Cortex workers | Rust workers (Temporal pattern, strictly typed API boundaries) |
| 4 | Institutional intelligence | Fully native, hardened execution topologies via OS-level sandboxing |
...
- **Migration posture**: `cortex-eudaemon` is the future parity target, moving to decoupled Rust crates to prevent core bloat (Phase 7+)
- **Security Posture**: Stage 4 institutional execution requires formalized host sandboxing to isolate tool evaluation
