...
### 7.5 Rust-Native Runtime Migration & Strict Protocol Enforcement
Transition the current `eudaemon-alpha` Python worker into the `cortex-eudaemon` Rust-native parity slice. Aligning with high-performance agent architectures (e.g. OpenAI Codex pattern), this phase mandates:
- **Core Decoupling**: Prevent monolithic bloat by breaking Eudaemon's capabilities into localized crates (vs expanding `cortex-core`).
- **Strict Payload Boundaries**: Enforce strict Discriminated Union serialization (`#[serde(tag = "type")]`) and camelCase conventions across all Gateway and A2UI data transfer objects.
- **Protocol Fidelity**: Eliminate ad-hoc optional structures across language boundaries to guarantee Typescript client safety.

### 7.6 OS-Level Execution Sandboxing
To safely expand Eudaemon Alpha's un-gated action generation loops and codebase contributions (Stage 4 constraint elimination):
- **Hardened Execution Topology**: Wrap agent tool execution within an OS-level sandbox (e.g., containerized runtimes or `sandbox-exec` equivalents) maximizing isolation.
- **Privilege Degradation**: Ensure Temporal activities and shell/code evaluators execute with explicit network and file-system restrictions, eliminating arbitrary host contamination risks.
- **Infrastructure Protection**: Formalize the "runtime host topology" boundary required for production execution, replacing the native `systemd` trust model used during Phase 6.
