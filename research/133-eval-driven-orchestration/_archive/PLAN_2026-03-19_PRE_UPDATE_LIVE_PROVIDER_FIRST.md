...
- **Mechanism**: A workflow emits an audit-unit manifest, an execution adapter submits the independent units to a batch backend, a meta-evaluation pass scores correctness/novelty/feasibility/contradiction density, and the resulting evidence is routed back to Eudaemon or the steward for synthesis.
