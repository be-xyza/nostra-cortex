...
## Objective
Operationalize SIQ from local contract checks into portfolio governance, CI enforcement (`observe -> softgate`), and read-only host intake for Cortex and contribution-graph consumers (legacy `initiative-graph` naming is deprecated alias only if still present).
...
4. Optional contribution-graph SIQ intake with deterministic ordering and backward-compatible fields (keep legacy alias behavior stable if still present).
...
- Keep legacy `initiative-graph` alias behavior backward-compatible (optional SIQ fields only); canonical contract is contribution-graph.
...
7. Contribution-graph can consume SIQ metadata without contract regression (legacy alias stays backward compatible if still present).
