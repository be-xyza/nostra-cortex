...
- Preflight + config governance:
  ...
  - `bash scripts/check_skill_policy.sh` (PASS)
- Gateway inventory + protocol contract alignment:
  - `python3 scripts/refresh_gateway_parity_fixtures.py` (refreshed inventory=175)
    - note: superseded by 2026-03-05 baseline refresh (inventory=180) after capturing additional `GET` routes present in `cortex-eudaemon`.
...
- Preflight + config governance:
  ...
  - `bash scripts/check_skill_policy.sh` (PASS)
- Gateway inventory + protocol contract alignment:
  - `python3 scripts/refresh_gateway_parity_fixtures.py` (refreshed inventory=180)
  - `bash scripts/check_gateway_parity_inventory_sync.sh` (PASS: inventory=180)
  - `bash scripts/check_gateway_protocol_contract_coverage.sh` (PASS: inventory=180 contract=180)
  - `bash scripts/check_gateway_contract_descriptors_strict.sh` (PASS: descriptors=180)
- Rust tests:
  - `cd cortex && CARGO_NET_OFFLINE=true CARGO_TARGET_DIR=/tmp/cortex-eudaemon-target cargo test -p cortex-eudaemon --test gateway_parity` (PASS)
