...
*   **New Logic**:
    ...
    *   Resolve `DPubManifest` (Ordered Array of Chapters).
    *   Fetch `Chapter` nodes from Graph (`node_provider`).
    *   Render stream of `PolymorphicBlock`s. (Note: A DPub chapter is an ordered array of `PolymorphicBlock` IDs, natively supporting A2UI widgets inline with Rich Text blocks).
