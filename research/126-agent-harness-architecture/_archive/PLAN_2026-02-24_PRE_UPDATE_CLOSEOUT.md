...
cargo test --manifest-path cortex/Cargo.toml -p cortex-agents test_agent_execution_record_serialization

# Verify Authority Level enforcement (L1 generates Proposal)
cargo test --manifest-path cortex/Cargo.toml -p cortex-runtime test_authority_ladder_enforcement

# Verify GlobalEvent envelope mapping contract for lifecycle emissions
cargo test --manifest-path cortex/Cargo.toml -p cortex-runtime test_agent_execution_global_event_mapping
```
