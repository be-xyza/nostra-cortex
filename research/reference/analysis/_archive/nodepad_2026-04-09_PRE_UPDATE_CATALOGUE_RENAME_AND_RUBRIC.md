...
|-----------------|---------------|--------|
| Multi-View Canvas (Graph/Tiling/Kanban) | A2UI Heap Mode / Spatial gallery experiments | Experimental in `cortex-web` |
...
### Implemented Now
- `cortex-web` already contains a React `SpatialHeapGrid` renderer and a `/gallery` route for layout comparison.
...
1. **Harden Existing Spatial Experiments**: Keep the React `SpatialHeapGrid` path, normalize gallery vocabulary to topology-oriented names inside `cortex-web`, and maintain narrow renderer tests.
