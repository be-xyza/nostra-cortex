...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Antigravity rule governance policy")
    parser.add_argument("--mode", choices=["observe", "softgate", "hardgate"], default=None)
    parser.add_argument(
        "--policy-path",
        default="shared/standards/antigravity_rule_policy.toml",
    ...
    skills_root = resolve_path(
        root,
        str(source.get("skills_root", "$HOME/.gemini/antigravity/skills")),
    ...
    if governance_failures:
        print("FAIL: antigravity rule policy governance errors")
        for msg in governance_failures:
            print(f" - {msg}")
    if required_fails or forbidden_fails:
        print("WARN: antigravity rule policy violations detected")
    ...
    if not governance_failures and not hits:
        print("PASS: antigravity rule policy checks")
