...
bash "$ROOT_DIR/scripts/run_repo_python.sh" "$ROOT_DIR/scripts/validate_standards_registry.py" --root "$ROOT_DIR"

bash "$ROOT_DIR/scripts/run_repo_python.sh" - "$ROOT_DIR" "$INDEX_PATH" <<'PY'
...
required_root_contracts = {
    ...
    "skill_policy.toml": root / "shared" / "standards" / "skill_policy.toml",
...
required_root_artifacts = {
    ...
    "skill_policy_dispositions.json": root / "shared" / "standards" / "skill_policy_dispositions.json",
