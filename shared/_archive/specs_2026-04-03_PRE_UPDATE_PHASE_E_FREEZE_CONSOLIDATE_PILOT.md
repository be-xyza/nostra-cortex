...
type RetrievalPolicy = {
    retrievalMode: Text; // "graph_only" | "hybrid_graph_embedding"
    maxHops: Nat;
...
Rule:
- `embeddingsManifestRef` remains optional at schema level, but is operationally required when `retrievalMode = "hybrid_graph_embedding"`.
...
type TripleQueryScope = {
    ...
    scopeRef: ?Text; // ResourceRef; required for actor/agent, forbidden for system, omitted for any
};
...
type TripleQueryRequest = {
    queryId: Text;
    orderingStrategy: Text; // "canonical" tuple order: graph scope, subject, predicate, object, provenance scope, source ref
...
type TripleMatch = {
    ...
    sourceRef: ?Text;
    confidence: ?Float; // Optional provenance enrichment; required lineage fields remain even when includeProvenance = false
};
