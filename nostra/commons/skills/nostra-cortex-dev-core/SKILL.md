...
1. Run preflight before implementation:
   ...
   - `bash scripts/check_skill_registry_integrity.sh` (required when skill governance surfaces are in scope)
   - `bash scripts/check_skill_policy.sh` (required when skill governance surfaces are in scope)
   - `bash scripts/check_workflow_declarations.sh` (required when workflow governance surfaces are in scope)
