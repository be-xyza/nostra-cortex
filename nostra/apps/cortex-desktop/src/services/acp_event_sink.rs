...
#[cfg(any(feature = "cortex_runtime_v0", test))]
...
impl AcpEventSink {
    ...
    fn build_runtime_orchestrator(&self) -> EventOrchestrator {
        ...
        EventOrchestrator::new(
            ...
            None,
            self.log_registry_endpoint.as_ref().map(|_| {
                Arc::new(DesktopNetworkAdapter) as Arc<dyn NetworkAdapter>
            }),
        )
    }
}
...
mod tests {
    ...
    #[cfg(feature = "cortex_runtime_v0")]
    use crate::services::acp_event_projector::{
        project_session_update_with_timestamp, to_domain_trace_context, to_domain_update_kind,
        to_cloud_event as legacy_to_cloud_event,
    };
    use crate::services::acp_meta_policy::TraceContext;
    #[cfg(feature = "cortex_runtime_v0")]
    use cortex_domain::events::{ProjectSessionUpdateInput, project_session_update as domain_project};
    #[cfg(feature = "cortex_runtime_v0")]
    ...
    async fn runtime_v0_matches_legacy_for_all_update_kinds() {
        ...
        for kind in all_update_kinds() {
            ...
            assert_eq!(runtime.payload, legacy.payload, "payload mismatch for {:?}", kind);
    ...
    fn cloud_event_parity_allows_timestamp_format_differences_only() {
        ...
        for kind in all_update_kinds() {
            ...
            assert_eq!(legacy_cloud.id, runtime_cloud.id, "id mismatch for {:?}", kind);
            assert_eq!(
            ...
            assert_eq!(legacy_cloud.type_, runtime_cloud.type_, "type mismatch for {:?}", kind);
            assert_eq!(legacy_cloud.data, runtime_cloud.data, "data mismatch for {:?}", kind);
            assert_eq!(
