...
pub fn project_session_update(input: ProjectSessionUpdateInput) -> Result<ProjectedEvent, DomainError> {
    if input.session_id.is_empty() {
        return Err(DomainError::InvalidInput("session_id must not be empty"));
    }

    Ok(ProjectedEvent {
        id: deterministic_event_id(&input.session_id, input.turn_seq, input.update_seq, input.kind),
