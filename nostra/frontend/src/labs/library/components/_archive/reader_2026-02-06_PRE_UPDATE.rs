use crate::labs::library::data::{
    dpub_path_for_id, edition_manifest_path_for_id, edition_snapshot_path_for_id,
    editions_dir_for_id, feed_path_for_id,
};
...
pub fn Reader(
    ...
    let publish_local = move |_| {
        ...
        if let Ok(bytes) = serde_json::to_vec(&updated) {
            let _ = vfs_mut.write_file(&dpub_path, bytes, "application/json");
        }

        // Chronicle append (local)
        let event_path = "/lib/chronicle/edition_published.jsonl";
        let mut existing = vfs_mut
            .read_file_string(event_path)
            .unwrap_or_default();
    ...
    let publish_canister = move |_| {
        ...
        spawn(async move {
            ...
            match api::publish_dpub_edition(&agent, dpub_path.clone(), version.clone(), name, override_token).await {
                Ok(_) => {
                    ...
                    if let Err(e) = sync_result {
                        publish_status.set(Some(format!("Published via canister; sync blocked: {}", e)));
                    } else {
                ...
                div { class: "absolute inset-0 z-50 bg-black/60 flex items-center justify-center p-4",
                    div { class: "w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl p-4 space-y-4",
                        ...
                        div { class: "text-xs text-muted-foreground",
                            "This publishes a local immutable edition (Merkle root + snapshot). Canister publishing is next."
                        }
